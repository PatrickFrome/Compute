(() => {
  ...
  async function setSupervisorMode(value, source = "LOCAL") {
    ...
    await addEvent(source, "MODE", `Supervisor mode → ${next}`, "info");
    return next;
  }

  async function seedSessionMode() {
    const x = await chrome.storage.session.get(MODE_KEY);
    if (!MODES.has(String(x[MODE_KEY] || "").toUpperCase())) {
      await chrome.storage.session.set({ [MODE_KEY]: "OFF" });
    }
  }
  ...
  async function resetSessionAuthority(reason) {
    ...
    await addEvent("SUPERVISOR", "AUTHORITY_RESET", `Session authority reset · ${reason}`, "info");
  }
  ...
  function semanticTargets(frame) {
    ...
    for (const node of rows) {
      ...
      const backendNodeId = Number(node?.backend_dom_node_id);
      if (!SAFE_REMOTE_SEMANTIC_ROLES.has(role) || !name || !Number.isInteger(backendNodeId)) continue;
      const key = `${role}\u0000${name}`;
      seen.set(key, Number(seen.get(key) || 0) + 1);
      out.push({ role, name: name.slice(0, 500), backend_node_id: backendNodeId });
    }
  ...
  async function localState() {
    const local = await chrome.storage.local.get([
      ...
      "snapshot:CHATGPT", "snapshot:GLM_ZAI", "a2BridgePendingCommandV0523",
      "a2BridgeGlmActuatedPredecessorV0523", "bridgeCommandFetchSuppressed",
      CURRENT_KEY, LAST_KEY, LAST_SYNC_KEY, LAST_ERROR_KEY
    ]);
    const session = await chrome.storage.session.get(["a2OperatorHeldPromptIntentV060", MODE_KEY]);
    const supervisorMode = MODES.has(String(session[MODE_KEY] || "").toUpperCase())
      ? String(session[MODE_KEY]).toUpperCase()
      : "OFF";
    const perception = perceptionSummary();
    const flatTargets = Object.entries(perception).flatMap(([platform, value]) =>
      (value.semantic_targets || []).map((target) => ({
        platform,
        ...target,
        perception_captured_at: value.captured_at
      }))
    );
    const intent = session.a2OperatorHeldPromptIntentV060 || null;
    return {
      ...
      bridge: {
        online_at: local.daemonOnlineAt || null,
        error: local.daemonLastError || null,
        command_fetch_suppressed: local.bridgeCommandFetchSuppressed || null
      },
      peers: {
      ...
      prompt_intent: intent?.intent_id ? {
      ...
      authority_effect: supervisorMode === "CONTROL" || local.armed === true
    };
  }

  async function heartbeat(lastCommand = null, lastStatus = null) {
    const state = await localState();
    const response = await supervisorRequest("/v1/state", {
      method: "POST",
      body: JSON.stringify({ state, last_command_id: lastCommand, last_command_status: lastStatus })
    });
    if (!response.ok) throw new Error(`supervisor_state_http_${response.status}`);
  ...
  async function broadcastPromptMode(next) {
    ...
    await Promise.allSettled(
      tabs
        .filter((tab) => Number.isInteger(tab?.id) && ["CHATGPT", "GLM_ZAI"].includes(platformOf(tab.url || "")))
  ...
  async function semantic(action, command) {
    ...
    if (matches.length !== 1) {
      throw new Error(matches.length ? `supervisor_semantic_target_ambiguous:${matches.length}` : "supervisor_semantic_target_not_found");
    }
    const message = {
      action,
      platform,
  ...
  async function execute(command) {
    ...
    if (currentMode !== "CONTROL" && !BOOTSTRAP_ACTIONS.has(action)) {
      throw new Error(`supervisor_control_required:${currentMode}`);
    }
    if (action === "SET_SUPERVISOR_MODE") {
      const next = await setSupervisorMode(command.payload?.mode, "REMOTE");
    ...
    if (action === "POLL") {
      if (typeof globalThis.A2_BRIDGE_POLL_NOW !== "function") throw new Error("supervisor_bridge_direct_poll_unavailable");
      return globalThis.A2_BRIDGE_POLL_NOW();
    }
    if (action === "CAPTURE") return capture(String(command.platform || ""));
    if (action === "STOP_GENERATION") {
      if (typeof globalThis.A2_OPERATOR_STOP_GENERATION !== "function") throw new Error("supervisor_stop_unavailable");
      return globalThis.A2_OPERATOR_STOP_GENERATION(String(command.platform || ""));
    }
    if (action === "SCROLL") {
      if (typeof globalThis.A2_OPERATOR_SCROLL !== "function") throw new Error("supervisor_scroll_unavailable");
      return globalThis.A2_OPERATOR_SCROLL(String(command.platform || ""), Number(command.payload?.delta_y || 0));
    }
  ...
  function authorityEffectFor(action, ok) {
    return ok === true && AUTHORITY_ACTIONS.has(String(action || "").toUpperCase());
  }

  async function postResult(command, ok, receipt, error = null) {
    const authorityEffect = authorityEffectFor(command.action, ok);
    const response = await supervisorRequest(`/v1/commands/${encodeURIComponent(command.command_id)}/result`, {
      method: "POST",
      body: JSON.stringify({
        ok,
        receipt: {
          schema: "metaengine.a2-browser-supervisor.receipt.v3",
    ...
    if (!response.ok) throw new Error(`supervisor_result_http_${response.status}`);
  ...
  async function poll() {
    ...
    pollPromise = (async () => {
      ...
      try {
        ...
        const response = await supervisorRequest("/v1/commands/next", {
          method: "POST",
          body: JSON.stringify({ supervisor_mode: state.supervisor_mode })
        });
        if (!response.ok) throw new Error(`supervisor_next_http_${response.status}`);
        const body = await response.json();
        current = body?.command || null;
        if (!current) return state;

        await chrome.storage.local.set({ [CURRENT_KEY]: current });
        await addEvent("CHAT", "COMMAND", `${current.action}${current.platform ? ` · ${current.platform}` : ""}`, "command", {
          command_id: current.command_id,
          idempotency_key: current.idempotency_key || null
        });

        let result = null;
        try {
          result = await execute(current);
          const authorityEffect = await postResult(current, true, result, null);
          const receipt = {
            command_id: current.command_id,
            idempotency_key: current.idempotency_key || null,
          ...
          await addEvent("EXTENSION", "RECEIPT", `${current.action} completed`, "success", {
            command_id: current.command_id,
            idempotency_key: current.idempotency_key || null,
            authority_effect: authorityEffect
          });
          ...
          await addEvent("EXTENSION", "ERROR", `${current.action}: ${message}`, "error", {
            command_id: current.command_id,
            idempotency_key: current.idempotency_key || null
          });
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (!["A2_SUPERVISOR_STATUS", "A2_SUPERVISOR_SET_MODE", "A2_SUPERVISOR_POLL_NOW", "A2_SUPERVISOR_CLEAR_EVENTS"].includes(type)) {
      return false;
    }
    if (!trustedSidePanel(sender)) {
      sendResponse({ ok: false, error: "supervisor_sender_not_trusted" });
      return false;
    }
    if (type === "A2_SUPERVISOR_STATUS") {
      status().then((state) => sendResponse({ ok: true, state })).catch((error) => sendResponse({ ok: false, error: String(error?.message || error) }));
      return true;
    }
    if (type === "A2_SUPERVISOR_SET_MODE") {
      setSupervisorMode(message?.mode, "LOCAL").then((next) => poll().catch(() => {}).then(() => sendResponse({ ok: true, mode: next }))).catch((error) => sendResponse({ ok: false, error: String(error?.message || error) }));
      return true;
    }
    if (type === "A2_SUPERVISOR_POLL_NOW") {
      poll().then((result) => sendResponse({ ok: true, result })).catch((error) => sendResponse({ ok: false, error: String(error?.message || error) }));
      return true;
    }
    if (type === "A2_SUPERVISOR_CLEAR_EVENTS") {
      chrome.storage.local.set({ [EVENTS_KEY]: [] }).then(() => sendResponse({ ok: true })).catch((error) => sendResponse({ ok: false, error: String(error?.message || error) }));
      return true;
    }
    return false;
  });
  ...
  chrome.runtime.onInstalled.addListener(() => {
    resetSessionAuthority("install").then(() => {
  ...
  chrome.runtime.onStartup.addListener(() => {
    resetSessionAuthority("browser_start").then(() => {
      chrome.alarms.create(ALARM, { periodInMinutes: 0.5 });
      return heartbeat();
    }).catch(() => {});
  });
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    if ((area === "local" && (changes.armed || changes.operatorMode)) || (area === "session" && changes[MODE_KEY])) {
      heartbeat().catch(() => {});
    }
  });
