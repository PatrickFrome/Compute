(() => {
  ...
  const CONTROL_ACTIONS = new Set(["ARM", "DISARM", "SET_MODE", "POLL", "CAPTURE", "STOP_GENERATION", "SCROLL", "SEMANTIC_FOCUS", "SEMANTIC_TYPE", "RESOLVE_PROMPT"]);
  ...
  async function seedSessionMode() {
    const x = await chrome.storage.session.get(MODE_KEY);
    if (!MODES.has(String(x[MODE_KEY] || ""))) await chrome.storage.session.set({ [MODE_KEY]: "OFF" });
  }
  ...
  function peerSummary(snapshot) {
    ...
    return { platform: snapshot.platform || null, url: normUrl(snapshot.url || ""), generating: snapshot.generating === true, message_count: Number(snapshot.message_count || 0), composer_present: snapshot.composer_present === true, composer_empty: normalize(snapshot.composer_text || "") === "", dom_pair_error: snapshot.dom_pair_error || null };
  }
  function semanticTargets(frame) {
    ...
    for (const node of rows) {
      ...
      out.push({ role, name: name.slice(0, 500), backend_node_id: Number(node.backend_dom_node_id) });
    }
  ...
  function perceptionSummary() {
    ...
    for (const platform of ["GLM_ZAI", "CHATGPT"]) {
      ...
      if (!frame) continue;
      result[platform] = { captured_at: frame.captured_at || null, url: normUrl(frame.url || ""), frame_token: frame.frame_token || null, body_text_sha256: frame.hashes?.body_text_sha256 || null, screenshot_sha256: frame.hashes?.screenshot_sha256 || null, body_excerpt: clip(frame.page?.body_text || "", 7000), semantic_targets: semanticTargets(frame) };
    }
    return result;
  }
  async function localState() {
    const local = await chrome.storage.local.get(["armed","operatorMode","daemonOnlineAt","daemonLastError","lastOrderingPolicy","snapshot:CHATGPT","snapshot:GLM_ZAI","a2BridgePendingCommandV0523","a2BridgeGlmActuatedPredecessorV0523","bridgeCommandFetchSuppressed",CURRENT_KEY,LAST_KEY,LAST_SYNC_KEY,LAST_ERROR_KEY]);
    const session = await chrome.storage.session.get(["a2OperatorHeldPromptIntentV060", MODE_KEY]);
    const supMode = MODES.has(String(session[MODE_KEY] || "")) ? String(session[MODE_KEY]) : "OFF";
    ...
    return {
      operator_runtime: globalThis.A2_OPERATOR_RUNTIME || "0.6.3-supervisor-fsm-dev.1",
      ...
      bridge: { online_at: local.daemonOnlineAt || null, error: local.daemonLastError || null, command_fetch_suppressed: local.bridgeCommandFetchSuppressed || null },
      peers: { GLM_ZAI: peerSummary(local["snapshot:GLM_ZAI"]?.snapshot), CHATGPT: peerSummary(local["snapshot:CHATGPT"]?.snapshot) },
      ordering: { predecessor_command_id: local.a2BridgeGlmActuatedPredecessorV0523 || null, pending_command: local.a2BridgePendingCommandV0523 || null },
      perception,
      semantic_targets: flatTargets.slice(0, 60),
      prompt_intent: session.a2OperatorHeldPromptIntentV060 ? { intent_id: session.a2OperatorHeldPromptIntentV060.intent_id, platform: session.a2OperatorHeldPromptIntentV060.platform, event_type: session.a2OperatorHeldPromptIntentV060.event_type, created_at: session.a2OperatorHeldPromptIntentV060.created_at, draft_sha256: session.a2OperatorHeldPromptIntentV060.draft_sha256, original_draft: clip(session.a2OperatorHeldPromptIntentV060.original_draft || "", 12000) } : null,
  ...
  async function broadcastPromptMode(next) {
    ...
    await Promise.allSettled(tabs.filter((tab) => Number.isInteger(tab?.id) && ["CHATGPT","GLM_ZAI"].includes(platformOf(tab.url || ""))).map((tab) => chrome.tabs.sendMessage(tab.id, { type: "A2_PROMPT_GATE_CONFIG", mode: next })));
  }
  ...
  async function capture(platform) {
    ...
    return { platform, captured_at: frame.captured_at, frame_token: frame.frame_token, body_text_sha256: frame.hashes?.body_text_sha256 || null, screenshot_sha256: frame.hashes?.screenshot_sha256 || null, body_excerpt: clip(frame.page?.body_text || "", 7000), semantic_targets: semanticTargets(frame) };
  }
  ...
  chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
    ...
    if (type==="A2_SUPERVISOR_SET_MODE") { setMode(message?.mode).then(mode=>poll().catch(()=>{}).then(()=>sendResponse({ok:true,mode}))).catch(e=>sendResponse({ok:false,error:String(e?.message||e)})); return true; }
    if (type==="A2_SUPERVISOR_POLL_NOW") { poll().then(result=>sendResponse({ok:true,result})).catch(e=>sendResponse({ok:false,error:String(e?.message||e)})); return true; }
  ...
  chrome.runtime.onInstalled.addListener(()=>{ resetSessionAuthority("install").then(()=>{ chrome.alarms.create(ALARM,{periodInMinutes:0.5}); return heartbeat(); }).catch(()=>{}); });
  chrome.runtime.onStartup.addListener(()=>{ resetSessionAuthority("browser_start").then(()=>{ chrome.alarms.create(ALARM,{periodInMinutes:0.5}); return heartbeat(); }).catch(()=>{}); });
  chrome.alarms.onAlarm.addListener((alarm)=>{ if(alarm.name===ALARM) poll().catch(()=>{}); });
  chrome.storage.onChanged.addListener((changes,area)=>{ if ((area==="local" && (changes.armed||changes.operatorMode)) || (area==="session" && changes[MODE_KEY])) heartbeat().catch(()=>{}); });

  globalThis.A2_SUPERVISOR_POLL = poll;
  globalThis.A2_SUPERVISOR_STATUS = status;

  (async()=>{ await seedSessionMode(); chrome.alarms.create(ALARM,{periodInMinutes:0.5}); await heartbeat().catch(()=>{}); })().catch(()=>{});
})();