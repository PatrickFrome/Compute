(() => {
  ...
  async function tabById(tabId) {
    ...
    try {
      ...
      const live = tab?.url || tab?.pendingUrl || "";
      return tab?.id && isChatgpt(live) ? tab : null;
    } catch (_) {
      return null;
    }
  }
  ...
  async function waitContent(tabId, timeout = CONTENT_READY_TIMEOUT_MS) {
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (response?.ok === true && response.snapshot?.composer_present === true && !response.snapshot?.dom_pair_error) return response.snapshot;
      } catch (_) {}
      await sleep(200);
    }
  ...
  async function createRoot(reason) {
    const now = Date.now();
    const existing = (await chrome.storage.local.get(CREATE_LEASE_KEY))[CREATE_LEASE_KEY] || null;
    const expires = Date.parse(String(existing?.expires_at || ""));
    if (Number.isInteger(Number(existing?.tab_id)) && Number.isFinite(expires) && expires > now) {
      const pending = await tabById(Number(existing.tab_id));
      if (pending) {
        const snapshot = await waitContent(Number(pending.id));
        await storeDirectSnapshot(Number(pending.id), snapshot);
        return chrome.tabs.get(Number(pending.id));
      }
    }
    const meta = await readMeta();
    const lease = { token: crypto.randomUUID(), tab_id: null, created_at: new Date().toISOString(), expires_at: new Date(now + CREATE_LEASE_MS).toISOString() };
    await chrome.storage.local.set({ [CREATE_LEASE_KEY]: lease });
    let tab;
    try {
      tab = await chrome.tabs.create({ url: CHATGPT_ROOT, active: false });
      if (!Number.isInteger(Number(tab?.id))) throw new Error("supervisor_chat_create_failed");
      lease.tab_id = Number(tab.id);
      await chrome.storage.local.set({ [CREATE_LEASE_KEY]: lease });
      const epoch = Math.max(1, meta.epoch + 1);
      const recoveryCount = Math.max(0, Number(meta.health?.recovery_count || 0));
      await chrome.storage.local.set({ [TAB_KEY]: Number(tab.id), [URL_KEY]: null, [EPOCH_KEY]: epoch });
      ...
      const current = (await chrome.storage.local.get(CREATE_LEASE_KEY))[CREATE_LEASE_KEY] || null;
      if (current?.token === lease.token) await chrome.storage.local.remove(CREATE_LEASE_KEY);
    }
  }

  async function adoptLiveUrl(tab, reason = "url_adopted") {
    const url = normUrl(tab?.url || tab?.pendingUrl || "");
  ...
  async function ensure(reason = "ensure") {
    ...
    ensurePromise = (async () => {
      ...
      await writeHealth(isConversation(effectiveUrl) ? "READY_CONVERSATION" : "READY_ROOT", reason, {
        tab_id: tab.id,
        url: isConversation(effectiveUrl) ? normUrl(effectiveUrl) : null,
        epoch: meta.epoch,
        recovery_count: Math.max(0, Number(meta.health?.recovery_count || 0)),
  ...
  async function maintainIfProvisioned(reason) {
    const meta = await readMeta();
    const lease = (await chrome.storage.local.get(CREATE_LEASE_KEY))[CREATE_LEASE_KEY] || null;
    if (meta.tab_id || lease?.tab_id) return ensure(reason);
    return { enabled: meta.enabled, state: "IDLE_NO_SUPERVISOR_TAB" };
  }

  chrome.alarms.onAlarm.addListener((alarm) => {
    if (alarm.name === ALARM) maintainIfProvisioned("alarm").catch(() => {});
  });

  chrome.runtime.onStartup.addListener(() => maintainIfProvisioned("browser_start").catch(() => {}));
  chrome.runtime.onInstalled.addListener(() => writeHealth("IDLE", "install_lazy", { recovery_count: 0 }).catch(() => {}));
