(() => {
  "use strict";
  const RUNTIME = "0.6.8-final.1";
  const originalRequest = globalThis.A2_BRIDGE_REQUEST;
  if (typeof originalRequest !== "function") throw new Error("v068_bridge_runtime_dependency_missing");
  globalThis.A2_BRIDGE_REQUEST = async (path, init = {}) => {
    ...
    return originalRequest(path, { ...init, body: JSON.stringify({ ...body, operator_runtime: RUNTIME }) });
  };
