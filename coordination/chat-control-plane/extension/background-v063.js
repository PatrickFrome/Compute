(() => {
  ...
  async function settings() {
    ...
    s.daemonUrl=String(s.daemonUrl||DEFAULTS.daemonUrl).replace(/\/+$/," ").trim();
    s.pollMs=Math.max(1000,Math.min(30000,Number(s.pollMs)||2500));
    return s;
  }
  ...
  async function reportSnapshot(tabId,snapshot) {
    ...
    try {
      const r=await request("/v1/snapshots",{method:"POST",body:JSON.stringify(envelope)});
  ...
  async function resolveTab(command,s) {
    ...
    if(normUrl(live.url||"")!==url)throw new Error("target_url_mismatch");
    return live;
  }
  ...
  async function rememberCompleted(c,result){
    const rows=await loadCompleted(),id=String(c.command_id||""),key=String(c.idempotency_key||"");
    const next=rows.filter((r)=>r.command_id!==id&&r.idempotency_key!==key);
    next.push({command_id:id,idempotency_key:key,target_platform:c.target_platform,result_status:result.status,target_url:result.target_url||null,execution_class:result.execution_class||null,completed_at:new Date().toISOString()});
    await chrome.storage.local.set({[COMPLETED_KEY]:next.slice(-MAX_COMPLETED)});
  }
  async function postResult(id,result){
    try{
      const r=await request(`/v1/commands/${encodeURIComponent(id)}/result`,{method:"POST",body:JSON.stringify(result)});
      if(!r.ok)throw new Error(`result_http_${r.status}`);
      return true;
    } catch(error) {
  ...
  async function waitNewConversation(tabId,timeout=12000){
    ...
    while(Date.now()<deadline){
      ...
      if(isChatgptConversation(tab?.url||"")){
        const snap=await waitSnapshot(tabId,1600).catch(()=>null);
        if(snap)return{tab,snap};
      }
      await sleep(150);
    }
    throw new Error("chatgpt_rollover_timeout");
  }
  async function sendChatgpt(tab,command) {
    try {
      const r=await globalThis.A2_CHATGPT_TRUSTED_SEND(tab.id,command);
      if(r?.ok!==true)throw Object.assign(new Error(r?.error||r?.status||"chatgpt_send_failed"),{a2Result:r});
      return r;
    } catch(error){
  ...
  async function execute(command) {
    ...
    if(prior){
      ...
      const accepted=await postResult(id,envelope);
      if(accepted){await clearPendingIf(id);await consumePredecessorIfSafe(command,envelope.execution_class,true);}
      return;
    }
    inFlight.add(id);
    try {
  ...
  async function resumePending(){
    const x=await chrome.storage.local.get(PENDING_KEY),c=x[PENDING_KEY];
    if(c?.command_id)await execute(c);
  }
  async function poll(force=false) {
    if(pollPromise)return pollPromise;
    pollPromise=(async()=>{
      const s=await settings();
      if(!force&&Date.now()-lastPollAt<s.pollMs)return;
      lastPollAt=Date.now();
      await resumePending();
      if(!s.armed){
        await pollSnapshots();
        await chrome.storage.local.set({daemonOnlineAt:new Date().toISOString(),daemonLastError:null,lastOrderingPolicy:ORDERING_POLICY,operatorRuntime:OPERATOR_RUNTIME,bridgeCommandFetchSuppressed:"DISARMED"});
        return {armed:false,command_fetch_suppressed:true};
      }
      try {
        ...
        await chrome.storage.local.set({daemonOnlineAt:new Date().toISOString(),daemonLastError:null,lastOrderingPolicy:body?.ordering_policy||ORDERING_POLICY,operatorRuntime:OPERATOR_RUNTIME,bridgeCommandFetchSuppressed:null});
        return body;
      } catch(error){
        await chrome.storage.local.set({daemonLastError:String(error?.message||error),daemonLastErrorAt:new Date().toISOString()});
        throw error;
  ...
  async function initialize(){
    await globalThis.A2_SECRET_VAULT_READY?.catch(()=>{});
    await globalThis.A2_BRIDGE_CLIENT_ID?.();
  ...
  chrome.runtime.onInstalled.addListener(async()=>{
    const old=await chrome.storage.local.get(Object.keys(DEFAULTS)),seed={};
    for(const[k,v]of Object.entries(DEFAULTS))if(old[k]===undefined)seed[k]=v;
  ...
  chrome.alarms.create("a2-chat-bridge-poll",{periodInMinutes:0.5});
  badge();
  resumePending().finally(()=>poll(true));
})();