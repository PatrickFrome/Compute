...
const LOOKUP_SCHEMA = 'metaengine.a2-compute-browser.planning-lookup.v4';
...
const DIRECT_ROUTING_INDEX_MAX_TOOLS = 8;
...
function sameCausalNamespace(left, right) {
  return left?.target_id === right?.target_id && left?.context_id === right?.context_id && Number(left?.conversation_epoch) === Number(right?.conversation_epoch) && left?.document_epoch === right?.document_epoch;
}
...
function plannerContextBytes(value) { return value == null ? 0 : Buffer.byteLength(JSON.stringify(value)); }
...
export class ComputePlanningBrokerService {
  ...
  async #routeColdLeader({ profileId, targetId, captured }) {
    try {
      ...
      if (catalog.status !== 'SUPPORTED') return { surface: 'SEMANTIC_PERCEPTION', semanticEnvelope: captured.envelope, routingIndex: null, searchHandle: null, degradedReason: 'WEBMCP_UNSUPPORTED' };
      if (catalog.tool_count === 0) return { surface: 'SEMANTIC_PERCEPTION', semanticEnvelope: captured.envelope, routingIndex: null, searchHandle: null, degradedReason: 'WEBMCP_NO_TOOLS' };
      if (catalog.tool_count <= DIRECT_ROUTING_INDEX_MAX_TOOLS) {
        ...
        return { surface: 'WEBMCP_ROUTING_INDEX', semanticEnvelope: null, routingIndex, searchHandle: null, degradedReason: null };
      }
      ...
      return { surface: 'WEBMCP_TOOL_SEARCH', semanticEnvelope: null, routingIndex: null, searchHandle, degradedReason: null };
    } catch (error) {
      if (fatalWebMcpRoutingError(error)) throw error;
      return { surface: 'SEMANTIC_PERCEPTION', semanticEnvelope: captured.envelope, routingIndex: null, searchHandle: null, degradedReason: 'WEBMCP_DISCOVERY_INVALID' };
    }
  }
  async lookup({ profileId, targetId, intentId, actionKind } = {}) {
    ...
    if (lookup.status !== 'MISS_LEADER') {
      return Object.freeze({ schema: LOOKUP_SCHEMA, lookup, planner_context_surface: 'NONE', planner_context_bytes: 0, planning_envelope: null, webmcp_routing_index: null, webmcp_search_handle: null, webmcp_degraded_reason: null, provider_neutral: true, model_execution_location: 'EXTERNAL_AGENT', authority_effect: false, web_authority_effect: false, actuation_eligible: false });
    }
    try {
      const routed = await this.#routeColdLeader({ profileId, targetId, captured });
      const chosen = routed.routingIndex || routed.searchHandle || routed.semanticEnvelope;
      return Object.freeze({ schema: LOOKUP_SCHEMA, lookup, planner_context_surface: routed.surface, planner_context_bytes: plannerContextBytes(chosen), planning_envelope: routed.semanticEnvelope, webmcp_routing_index: routed.routingIndex, webmcp_search_handle: routed.searchHandle, webmcp_degraded_reason: routed.degradedReason, provider_neutral: true, model_execution_location: 'EXTERNAL_AGENT', authority_effect: false, web_authority_effect: false, actuation_eligible: false });
    } catch (error) {
...
export const COMPUTE_PLANNING_DIRECT_ROUTING_INDEX_MAX_TOOLS = DIRECT_ROUTING_INDEX_MAX_TOOLS;
