...
import { ComputePlanningBrokerService, COMPUTE_PLANNING_DIRECT_ROUTING_INDEX_MAX_TOOLS } from '../src/planning-broker-service.mjs';
...
function axFixture() { return { nodes: [{ nodeId: 'ax-root', ignored: false, role: { type: 'role', value: 'RootWebArea' }, name: { type: 'computedString', value: '' }, backendDOMNodeId: 101 }, { nodeId: 'ax-button', parentId: 'ax-root', ignored: false, role: { type: 'role', value: 'button' }, name: { type: 'computedString', value: 'Click me' }, description: { type: 'computedString', value: 'Safe action' }, properties: [{ name: 'focusable', value: { type: 'booleanOrUndefined', value: true } }], backendDOMNodeId: 103 }] }; }
...
function rawTool(index) {
  const named = [['search_flights','Search flight schedules by origin and destination.'],['book_flight','Book a selected airline itinerary.'],['search_hotels','Search hotels by city and date.']][index];
...
function supportedCatalog({ count = 3, documentEpoch = currentDocumentEpoch() } = {}) { return compileWebMcpCatalog(webMcpEnvelopeFromCdpTools(Array.from({ length: count }, (_, index) => rawTool(index)), { targetId, contextId, conversationEpoch, documentEpoch, mainFrameId: 'frame-main', capturedAt: '2026-08-28T15:30:00.000Z' })); }
function unsupportedCatalog() { return compileWebMcpCatalog(unsupportedWebMcpEnvelope({ targetId, contextId, conversationEpoch, documentEpoch: currentDocumentEpoch(), capturedAt: '2026-08-28T15:30:00.000Z', reason: 'CDP_WEBMCP_UNAVAILABLE' })); }

test('small WebMCP toolsets skip the extra search round-trip and receive only compact routing index', async () => {
  assert.equal(COMPUTE_PLANNING_DIRECT_ROUTING_INDEX_MAX_TOOLS, 8);
  const { runtime } = runtimeFixture();
  const service = new ComputePlanningBrokerService(runtime, { webMcpService: { catalog: async () => supportedCatalog({ count: 8 }) } });
  const leader = await service.lookup({ profileId, targetId, intentId: 'small', actionKind: 'CLICK' });
  assert.equal(leader.lookup.status, 'MISS_LEADER');
  assert.equal(leader.planner_context_surface, 'WEBMCP_ROUTING_INDEX');
  assert.equal(leader.webmcp_routing_index.tool_count, 8);
  assert.equal(leader.webmcp_search_handle, null);
  assert.equal(leader.planning_envelope, null);
  assert.ok(leader.planner_context_bytes < 48 * 1024);
  assert.equal(JSON.stringify(leader.webmcp_routing_index).includes('"input_schema":'), false);
  assert.equal(JSON.stringify(leader.webmcp_routing_index).includes('"annotations":'), false);
  service.abort({ profileId, flightId: leader.lookup.flight_id, leaseToken: leader.lookup.lease_token, reasonCode: 'TEST_DONE' });
});

test('large toolsets receive tiny search handle, then exact lease gets bounded fresh top-K matches', async () => {
  const { runtime } = runtimeFixture();
  let catalogCalls = 0;
  const webMcpService = { catalog: async () => { catalogCalls += 1; return supportedCatalog({ count: 32 }); } };
  ...
  assert.equal(leader.webmcp_search_handle.tool_count, 32);
  ...
  assert.equal(JSON.stringify(searched.search_result).includes('"input_schema":'), false);
  assert.equal(JSON.stringify(searched.search_result).includes('"annotations":'), false);
  assert.equal(catalogCalls, 2);
  service.abort({ profileId, flightId: leader.lookup.flight_id, leaseToken: leader.lookup.lease_token, reasonCode: 'TEST_DONE' });
});

test('invalid tool-search lease is rejected before fresh WebMCP browser work', async () => {
  const { runtime } = runtimeFixture(); let catalogCalls = 0;
  const service = new ComputePlanningBrokerService(runtime, { webMcpService: { catalog: async () => { catalogCalls += 1; return supportedCatalog({ count: 32 }); } } });
...
test('tool-search lease expiry during fresh catalog work is rejected by postflight fence', async () => {
  const { runtime } = runtimeFixture(); let now = 0; let catalogCalls = 0;
  const brokerFactory = () => new SemanticPlanningBroker({ clock: () => now, leaseTtlMs: 10 });
  const webMcpService = { catalog: async () => { catalogCalls += 1; if (catalogCalls === 2) now = 11; return supportedCatalog({ count: 32 }); } };
...
test('tool search NO_MATCH does not guess and leaves semantic fallback available', async () => {
  const { runtime } = runtimeFixture();
  const service = new ComputePlanningBrokerService(runtime, { webMcpService: { catalog: async () => supportedCatalog({ count: 32 }) } });
...
test('unsupported, empty, and malformed WebMCP degrade to semantic perception without authority', async () => {
  ...
  for (const [intentSuffix, webMcpService] of cases) {
    const { runtime } = runtimeFixture(); const service = new ComputePlanningBrokerService(runtime, { webMcpService });
    const result = await service.lookup({ profileId, targetId, intentId: `intent_${intentSuffix}`, actionKind: 'CLICK' });
    assert.equal(result.lookup.status, 'MISS_LEADER'); assert.equal(result.planner_context_surface, 'SEMANTIC_PERCEPTION'); assert.ok(result.planning_envelope?.nodes?.length > 0); assert.equal(result.webmcp_routing_index, null); assert.equal(result.webmcp_search_handle, null); assert.equal(result.authority_effect, false);
...
test('waiters receive no planner context and promoted cache hits continue to skip model context', async () => {
  const { runtime } = runtimeFixture(); const service = new ComputePlanningBrokerService(runtime, { webMcpService: { catalog: async () => supportedCatalog({ count: 2 }) } });
  ...
  assert.equal(waiter.lookup.status, 'WAIT_FOR_PROMOTION'); assert.equal(waiter.planner_context_surface, 'NONE'); assert.equal(waiter.planner_context_bytes, 0); assert.equal(waiter.planning_envelope, null); assert.equal(waiter.webmcp_routing_index, null); assert.equal(waiter.webmcp_search_handle, null);
  ...
  assert.equal(hit.lookup.status, 'HIT_REVALIDATED'); assert.equal(hit.lookup.model_call_required, false); assert.equal(hit.planner_context_surface, 'NONE'); assert.equal(hit.planner_context_bytes, 0); assert.equal(hit.planning_envelope, null); assert.equal(hit.webmcp_routing_index, null); assert.equal(hit.webmcp_search_handle, null);
});
...
test('invalid context lease is rejected before B4 capture, valid context still has post-capture namespace revalidation', async () => {
  const { runtime, scheduler } = runtimeFixture(); const service = new ComputePlanningBrokerService(runtime, { webMcpService: { catalog: async () => supportedCatalog({ count: 2 }) } });
  const leader = await service.lookup({ profileId, targetId, intentId: 'fallback', actionKind: 'CLICK' }); const callsBeforeInvalid = scheduler.calls.length;
  ...
  assert.equal(fresh.revalidation.status, 'CONTEXT_REVALIDATED'); assert.equal(fresh.fresh_capture_used, true); assert.equal(fresh.lease_preflight_used, true); assert.equal(fresh.lease_bound, true);
  scheduler.loaderId = 'loader-b'; await assert.rejects(service.context({ profileId, targetId, flightId: leader.lookup.flight_id, leaseToken: leader.lookup.lease_token, surface: 'SEMANTIC_PERCEPTION' }), /semantic_planning_broker_namespace_changed/);
  scheduler.loaderId = 'loader-a'; service.abort({ profileId, flightId: leader.lookup.flight_id, leaseToken: leader.lookup.lease_token, reasonCode: 'TEST_DONE' });
});

test('fatal document mismatch during initial routing aborts unseen flight so the next caller can lead', async () => {
  const { runtime } = runtimeFixture(); let mismatched = true;
  ...
  await assert.rejects(service.lookup({ profileId, targetId, intentId: 'recover', actionKind: 'CLICK' }), /planning_routing_namespace_changed/); assert.equal(service.stats({ profileId }).broker.in_flight_count, 0);
  mismatched = false; const retry = await service.lookup({ profileId, targetId, intentId: 'recover', actionKind: 'CLICK' });
  assert.equal(retry.lookup.status, 'MISS_LEADER'); assert.equal(retry.planner_context_surface, 'WEBMCP_ROUTING_INDEX');
  service.abort({ profileId, flightId: retry.lookup.flight_id, leaseToken: retry.lookup.lease_token, reasonCode: 'TEST_DONE' });
});

test('document change during deferred tool search aborts stale flight and requires new leader', async () => {
  const { runtime } = runtimeFixture(); let changed = false;
  const service = new ComputePlanningBrokerService(runtime, { webMcpService: { catalog: async () => supportedCatalog({ documentEpoch: changed ? 'doc_changed' : currentDocumentEpoch(), count: 32 }) } });
  const leader = await service.lookup({ profileId, targetId, intentId: 'search_drift', actionKind: 'CLICK' }); assert.equal(leader.planner_context_surface, 'WEBMCP_TOOL_SEARCH');
  changed = true; await assert.rejects(service.searchTools({ profileId, targetId, flightId: leader.lookup.flight_id, leaseToken: leader.lookup.lease_token, query: 'flight' }), /planning_tool_search_namespace_changed/);
  assert.equal(service.stats({ profileId }).broker.in_flight_count, 0);
});
