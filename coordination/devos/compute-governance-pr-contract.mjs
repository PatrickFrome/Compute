...
function checkedEvidence(body, label) {
  ...
  return re.test(String(body ?? '').replace(/\r\n/g, '\n'));
}
...
export function evaluateComputeGovernancePr({
  ...
  const fields = Object.fromEntries(
    [...CORE_FIELDS, ...READY_FIELDS].map((label) => [label, scalarField(body, label)]),
  );
  ...
  const missingEvidence = REQUIRED_EVIDENCE.filter((label) => !checkedEvidence(body, label));
