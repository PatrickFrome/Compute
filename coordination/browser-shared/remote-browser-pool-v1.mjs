export const REMOTE_BROWSER_POOL_VERSION = '1.1.0';
...
const ACTIVE_STATES = new Set(['RESERVED','IN_FLIGHT']);
...
export class RemoteBrowserPoolV1 {
  ...
  registerNode(input){
    ...
    if(!this.#activeNodeCounts.has(node.node_id)) this.#activeNodeCounts.set(node.node_id,0);
    return clone(node);
  }
  ...
  acquireLease({lease_id,action_id,resource_id,required_capabilities=[],now_ms,ttl_ms=120000}={}){
    ...
    if(this.#activeResourceLease.has(resourceId)) throw new RemoteBrowserPoolError('pool_resource_already_leased');
  ...
  completeLease({lease_id,outcome,reason_code='EXECUTOR_RECEIPT'}={}){
    const lease=this.#requireLease(lease_id);
    if(!ACTIVE_STATES.has(lease.state)) throw new RemoteBrowserPoolError('pool_lease_terminal');
  ...
  sweepExpired(now_ms){
    ...
    for(const leaseId of [...this.#activeLeaseIds]){
      const lease=this.#leases.get(leaseId);
      if(lease && now>=lease.expires_at_ms){
  ...
  activeSnapshot(){
    return freeze({
      version:REMOTE_BROWSER_POOL_VERSION,
      nodes:[...this.#nodes.values()].map(clone),
      leases:[...this.#activeLeaseIds].map((id)=>clone(this.#leases.get(id))).filter(Boolean),
      terminal_history_retained:true,
      auth_state_migration:false,
      raw_engine_transport_exposed:false,
  ...
  #activeNodeLeases(nodeId){ return this.#activeNodeCounts.get(nodeId)||0; }
  #activateLease(lease){
    if(!ACTIVE_STATES.has(lease.state)) throw new RemoteBrowserPoolError('pool_active_index_state_invalid');
    if(this.#activeLeaseIds.has(lease.lease_id)||this.#activeResourceLease.has(lease.resource_id)) throw new RemoteBrowserPoolError('pool_active_index_conflict');
    this.#activeLeaseIds.add(lease.lease_id);
    this.#activeResourceLease.set(lease.resource_id,lease.lease_id);
    this.#activeNodeCounts.set(lease.node_id,this.#activeNodeLeases(lease.node_id)+1);
  }
  #deactivateLease(lease){
    if(!this.#activeLeaseIds.delete(lease.lease_id)) return;
    if(this.#activeResourceLease.get(lease.resource_id)===lease.lease_id) this.#activeResourceLease.delete(lease.resource_id);
    const count=this.#activeNodeLeases(lease.node_id);
    if(count<=0) throw new RemoteBrowserPoolError('pool_active_node_count_underflow');
    this.#activeNodeCounts.set(lease.node_id,count-1);
  }
  #terminateNodeLeases(node,reason){
    for(const leaseId of [...this.#activeLeaseIds]){
      const lease=this.#leases.get(leaseId);
      if(lease&&lease.node_id===node.node_id&&lease.node_epoch===node.node_epoch&&lease.process_incarnation_id===node.process_incarnation_id){
        this.#terminalize(lease,lease.state==='IN_FLIGHT'?'AMBIGUOUS':'NO_EFFECT',reason);
      }
    }
  }
