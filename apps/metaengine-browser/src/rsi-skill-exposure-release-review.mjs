...
function verifyAdmissionProvenance(row,library,governance,skillDigest){
  ...
  const core={
    ...
    admission_attempt_digest:exactDigest(row.admission_attempt_digest,'admission_provenance_attempt'),
    ...
    effect_executor_identity_digest:exactDigest(row.effect_executor_identity_digest,'admission_provenance_effect_executor'),
    idempotency_key_digest:exactDigest(row.idempotency_key_digest,'admission_provenance_idempotency'),
    ...
    execution_authority:row.execution_authority,browser_authority:row.browser_authority,task_authority:row.task_authority,
    scheduler_authority:row.scheduler_authority,production_mutation_authority:row.production_mutation_authority,
    promotion_authority:row.promotion_authority,self_update_authority:row.self_update_authority,
    automatic_retry_allowed:row.automatic_retry_allowed,authority_effect:row.authority_effect,
  };
  if(core.skill_digest!==skillDigest
    ...
    ||core.exposure_hold_observed!==true||core.dormant_cap_observed!==true
    ||core.active_for_composition!==false||core.retrieval_exposure_allowed!==false){
...
export function createRsiSkillExposureReleaseReview({
  ...
  if(reviewerIdentities.includes(admissionProvenance.effect_executor_identity_digest)){
    throw new Error('rsi_exposure_review_admission_executor_reviewer_separation_required');
  }
  ...
  const core=zero({
    ...
    admission_attempt_digest:admissionProvenance.admission_attempt_digest,
    ...
    admission_effect_executor_identity_digest:admissionProvenance.effect_executor_identity_digest,
    ...
    admission_effect_executor_reviewer_separation_required:true,
...
export function verifyRsiSkillExposureReleaseReview(review,args={}){
  ...
  if(review.external_governance_reviewer!==true||review.external_matched_evaluator!==true||review.external_security_reviewer!==true
    ...
    ||review.admission_effect_executor_reviewer_separation_required!==true
...
export function rsiSkillExposureReleaseReviewTrustRootSnapshot(){
  const root=zero({
    ...
    confirmed_storage_admission_provenance_required:true,
    admission_attempt_digest_binding_required:true,
    admission_effect_executor_reviewer_separation_required:true,
    exact_consumer_scope_required:true,
    matched_same_instances_required:true,
    no_skill_or_matched_reference_required:true,
