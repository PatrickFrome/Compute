...
function exactDigest(v,l){
  ...
  if(!SHA256_RE.test(x))throw new Error(`rsi_exposure_release_${l}_digest_invalid`);
  return x;
}
function id(v,l){
  const x=String(v||'').trim();
  if(!SAFE_ID_RE.test(x))throw new Error(`rsi_exposure_release_${l}_invalid`);
  return x;
}
function positiveInt(v,l,max=1000000){
  const n=Number(v);
  if(!Number.isSafeInteger(n)||n<1||n>max)throw new Error(`rsi_exposure_release_${l}_invalid`);
  return n;
}
function zero(extra={}){
...
function assertZero(v,l){
  ...
  ]) if(v?.[f]!==false)throw new Error(`rsi_exposure_release_${l}_${f}_invalid`);
  if(v?.automatic_retry_allowed!==false)throw new Error(`rsi_exposure_release_${l}_automatic_retry_invalid`);
}
function holds(governance){
  ...
  return raw.map(x=>exactDigest(x,'governance_hold')).sort();
}
function entryCore(row){
...
function assertSameNonTargetEntries(current,next,target){
  ...
  for(const row of current.entries){
    ...
    if(!peer)throw new Error('rsi_exposure_release_preview_entry_set_drift');
    if(row.skill_digest===target)continue;
    if(JSON.stringify(stable(entryCore(row)))!==JSON.stringify(stable(entryCore(peer)))
      ||row.state!==peer.state||row.active_for_composition!==peer.active_for_composition){
...
export function createRsiSkillExposureReleasePreview({
  ...
  if(current.library_digest!==next.library_digest||current.library_id!==next.library_id){
    throw new Error('rsi_exposure_release_preview_library_drift');
  }
  const skillDigest=exactDigest(skill_digest,'skill');
  const currentHolds=holds(current);
  const nextHolds=holds(next);
  ...
  const expectedNext=currentHolds.filter(x=>x!==skillDigest);
  if(JSON.stringify(expectedNext)!==JSON.stringify(nextHolds)){
  ...
  if(JSON.stringify(stable(entryCore(currentRow)))!==JSON.stringify(stable(entryCore(nextRow)))){
    throw new Error('rsi_exposure_release_preview_target_evidence_drift');
  }
  assertSameNonTargetEntries(current,next,skillDigest);
  if(next.active_count!==current.active_count+1||next.dormant_count!==current.dormant_count-1){
    throw new Error('rsi_exposure_release_preview_count_delta_invalid');
  }
  ...
  const core=zero({
    ...
    current_active_for_composition:false,
    next_active_for_composition:true,
    current_admission_exposure_hold:true,
    next_admission_exposure_hold:false,
    current_hold_count:currentHolds.length,
    next_hold_count:nextHolds.length,
    hold_count_delta:-1,
    active_count_delta:1,
    only_target_routing_state_changed:true,
...
export function verifyRsiSkillExposureReleasePreview(preview,args={}){
  ...
  if(preview.preview_only!==true||preview.exposure_effect_performed!==false||preview.release_authorized!==false
    ||preview.release_mode!==RELEASE_MODE||preview.only_target_routing_state_changed!==true
...
export function createRsiSkillExposureReleaseCertificate({
  ...
  if(preview.skill_digest!==skillDigest)throw new Error('rsi_exposure_release_certificate_skill_binding_mismatch');
  if(external_governance_owner!==true||external_shadow_evaluator!==true
    ||external_security_reviewer!==true||external_canary_evaluator!==true||authored_by_candidate!==false){
  ...
  const blockers=[];
  if(count<MIN_SHADOW_CONTEXTS)blockers.push('INSUFFICIENT_SHADOW_CONTEXTS');
  ...
  const core=zero({
    ...
    retrieval_profile_digest:evidenceRoots[1],
    ...
    memory_poisoning_scan_digest:evidenceRoots[5],
    source_grounding_receipt_digest:evidenceRoots[6],
    bounded_canary_policy_digest:evidenceRoots[7],
    bounded_canary_result_digest:evidenceRoots[8],
    negative_transfer_memory_digest:evidenceRoots[9],
    ...
    canary_effect_mode:String(canary_effect_mode||'').trim().toUpperCase(),
    ...
    external_governance_owner_identity_digest:identities[0],
    external_shadow_evaluator_identity_digest:identities[1],
    external_security_reviewer_identity_digest:identities[2],
    external_canary_evaluator_identity_digest:identities[3],
    external_governance_owner:true,
    ...
    only_target_routing_state_may_change:true,
...
export function verifyRsiSkillExposureReleaseCertificate(certificate,args={}){
  ...
  if(certificate.release_mode!==RELEASE_MODE||certificate.held_skill_required!==true
    ||certificate.current_state_must_be_dormant!==true||certificate.next_state_must_be_exploration_active!==true
    ||certificate.only_target_routing_state_may_change!==true||certificate.automatic_full_activation_allowed!==false
    ||certificate.release_does_not_grant_browser_authority!==true||certificate.release_does_not_grant_tool_authority!==true
    ||certificate.one_attempt_release_required!==true||certificate.ambiguous_release_retry_allowed!==false
    ||certificate.exposure_effect_performed!==false||certificate.release_token!==null
    ||certificate.external_governance_owner!==true||certificate.external_shadow_evaluator!==true
    ||certificate.external_security_reviewer!==true||certificate.external_canary_evaluator!==true
...
export function rsiSkillExposureReleaseTrustRootSnapshot(){
  const root={
    schema:'metaengine.rsi.skill-exposure-release-root.v1',
    version:1,
    ...
    only_target_routing_state_may_change:true,
    minimum_shadow_context_count:MIN_SHADOW_CONTEXTS,
    ...
    automatic_full_activation_allowed:false,
    certificate_performs_exposure_effect:false,
    runtime_hold_release_implemented:false,
    one_attempt_release_required:true,
    ambiguous_release_retry_allowed:false,
    execution_authority:false,
    browser_authority:false,
