...
export class RsiMetaProfileCanaryReviewLedger{
  ...
  constructor({statePath,source_sha}={}){if(!statePath)throw new Error('rsi_canary_review_ledger_path_required');this.#path=path.resolve(statePath);this.#sourceSha=String(source_sha||'').trim().toLowerCase()}
  ...
  async add(review){if(!this.#initialized)throw new Error('rsi_canary_review_ledger_not_initialized');if(!review||review.schema!==RSI_META_PROFILE_CANARY_REVIEW_SCHEMA)throw new Error('rsi_canary_review_invalid');assertZero(review,'review');if(review.source_sha!==this.#sourceSha)throw new Error('rsi_canary_review_source_mismatch');const rc=structuredClone(review);delete rc.review_digest;if(digest(rc)!==exactDigest(review.review_digest,'review'))throw new Error('rsi_canary_review_digest_mismatch');const existing=this.#rows.find(x=>x.review_id===review.review_id||x.binding_digest===review.binding_digest);if(existing){if(existing.review_digest!==review.review_digest)throw new Error('rsi_canary_review_conflict');return zero({state:'IDEMPOTENT',review_digest:review.review_digest})}const expected=this.#rows.length+1;if(review.confirmation_index!==expected)throw new Error('rsi_canary_review_confirmation_index_out_of_sequence');const allocation=rsiRiskAllocationForConfirmation(RISK_BUDGET,expected);if(Number(review.alpha_used)<=0||Number(review.alpha_used)-allocation>EPS)throw new Error('rsi_canary_review_alpha_over_budget');const cumulative=this.#rows.reduce((s,x)=>s+Number(x.alpha_used),0)+Number(review.alpha_used);if(cumulative-RISK_BUDGET.global_alpha>EPS)throw new Error('rsi_canary_review_global_alpha_exhausted');if(this.#rows.length>=MAX_ROWS)throw new Error('rsi_canary_review_ledger_capacity_exceeded');this.#rows.push(structuredClone(review));await this.#persist();return zero({state:review.state,review_digest:review.review_digest})}
