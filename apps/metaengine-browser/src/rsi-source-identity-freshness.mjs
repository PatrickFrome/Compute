...
function exactSha(value,label){
  ...
  if(!SHA40_RE.test(out)) throw new Error(`rsi_source_freshness_${label}_sha_invalid`);
  return out;
}
...
function readbackTimeBlockers(readbacks,evaluated){
  const blockers=[];
  const pairs=[
...
function normalizeGithub(row,evidence){
  ...
  if(exactSha(row.head_sha,'github_head')!==evidence.github_source_sha) throw new Error('rsi_source_freshness_github_head_mismatch');
...
function normalizeDb(row,evidence){
  ...
  if(exactSha(row.baseline_sha,'db_baseline')!==evidence.db_authority_baseline_sha) throw new Error('rsi_source_freshness_db_baseline_mismatch');
...
function normalizeRuntime(row,evidence){
  ...
  const incarnation=boundedId(row.process_incarnation_id,'runtime_process_incarnation');
  if(exactSha(row.target_git_sha,'runtime_target_git')!==evidence.runtime_target_git_sha) throw new Error('rsi_source_freshness_runtime_target_mismatch');
  ...
  instant(row.read_at,'runtime_read_at');
...
export function createRsiFreshSourceIdentityConvergenceCertificate({
  ...
  const blockers=readbackTimeBlockers(readbacks,evaluated);
  ...
  const core=zero({
    ...
    convergence_evidence_digest:evidence.evidence_digest,
    source_identity_converged:evidence.source_identity_converged===true,
    readbacks,
...
export function verifyRsiFreshSourceIdentityConvergenceCertificate(certificate){
  ...
  const readbacks=plain(certificate.readbacks,'readbacks');
  const evidenceLike={
    schema:'metaengine.rsi.source-identity-convergence.v1',
    version:1,
  };
  void evidenceLike;
  const core=structuredClone(certificate);
  delete core.certificate_digest;
  if(exactDigest(certificate.certificate_digest,'certificate')!==digest(core)){
    throw new Error('rsi_source_freshness_certificate_digest_mismatch');
  }
  if(certificate.authored_by_candidate!==false) throw new Error('rsi_source_freshness_certificate_origin_invalid');
  if(certificate.freshness_policy?.caller_configurable!==false
    ||certificate.freshness_policy?.max_readback_age_ms!==MAX_READBACK_AGE_MS
    ||certificate.freshness_policy?.max_readback_span_ms!==MAX_READBACK_SPAN_MS
    ||certificate.freshness_policy?.max_future_skew_ms!==MAX_FUTURE_SKEW_MS
    ||certificate.freshness_policy?.runtime_incarnation_required!==true
    ||certificate.freshness_policy?.typed_source_kind_required!==true){
    throw new Error('rsi_source_freshness_policy_tampered');
  }
  plain(readbacks.github,'github_readback');
  plain(readbacks.db_authority,'db_readback');
  plain(readbacks.runtime,'runtime_readback');
