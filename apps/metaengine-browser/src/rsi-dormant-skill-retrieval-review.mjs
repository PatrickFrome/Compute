...
function verifyAdmissionReadback({ admission_attempt, successor_library, current_governance } = {}) {
  ...
  if (
    ...
    || skill.admission_exposure_hold_external_release_required !== true
...
export function createRsiDormantSkillRetrievalReview({
  ...
  const core = zero({
    ...
    admission_effect_executor_identity_digest: exactDigest(
      storage.admission_attempt.effect_executor_identity_digest,
      'admission_effect_executor_identity',
    ),
