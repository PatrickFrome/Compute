...
function positiveInt(value, label, max = 1000000) {
  ...
  if (!Number.isSafeInteger(out) || out < 1 || out > max) {
    throw new Error('rsi_dormant_review_' + label + '_invalid');
  }
  return out;
}

function nonNegativeInt(value, label, max = 1000000) {
  const out = Number(value);
...
function verifyAdmissionReadback({ admission_attempt, successor_library, current_governance } = {}) {
  ...
  if (
    ...
    || skill.admission_exposure_held !== true
...
export function createRsiDormantSkillRetrievalReview({
  ...
  const negativeTransfers = nonNegativeInt(negative_transfer_count, 'negative_transfer_count', pairCount);
  if (repairs + regressions > pairCount) throw new Error('rsi_dormant_review_pair_delta_count_invalid');
  if (skillSuccess - referenceSuccess !== repairs - regressions) {
    throw new Error('rsi_dormant_review_paired_success_delta_mismatch');
  }
  const tokenBudget = positiveInt(context_token_budget, 'context_token_budget', 1000000);
  const controlTokens = nonNegativeInt(control_token_cost, 'control_token_cost', tokenBudget);
  const treatmentTokens = nonNegativeInt(treatment_token_cost, 'treatment_token_cost', tokenBudget);
  const latencyBudget = positiveInt(latency_budget_ms, 'latency_budget_ms', 3600000);
  const controlLatency = nonNegativeInt(control_latency_ms, 'control_latency_ms', latencyBudget);
  const treatmentLatency = nonNegativeInt(treatment_latency_ms, 'treatment_latency_ms', latencyBudget);
  const pairedFunctionalNonRegression = regressions === 0 && hardFailures === 0 && negativeTransfers === 0;
  const verifiedMatchedGain = pairCount >= 3 && repairs >= 1 && skillSuccess > referenceSuccess;
  const budgetObservedPass = controlTokens <= tokenBudget
    && treatmentTokens <= tokenBudget
    && controlLatency <= latencyBudget
    && treatmentLatency <= latencyBudget;
  ...
  if (pairCount < 3) evidenceBlockers.push('INSUFFICIENT_MATCHED_PAIRS');
  if (budgetObservedPass !== true) evidenceBlockers.push('OBSERVED_BUDGET_EXCEEDED');
  ...
  const state = classify({
    ...
    task_non_regression: task_non_regression === true && pairedFunctionalNonRegression,
    safety_non_regression,
    security_non_regression,
    process_non_regression,
    outcome_non_regression,
    efficiency_non_regression,
    strict_post_append_improvement: strict_post_append_improvement === true && verifiedMatchedGain,
    coalition_ablation_pass,
    marginal_contribution_pass,
    active_cap_pass,
  ...
  const core = zero({
    ...
    negative_transfer_count: negativeTransfers,
    context_token_budget: tokenBudget,
    control_token_cost: controlTokens,
    treatment_token_cost: treatmentTokens,
    latency_budget_ms: latencyBudget,
    control_latency_ms: controlLatency,
    treatment_latency_ms: treatmentLatency,
    paired_functional_non_regression: pairedFunctionalNonRegression,
    verified_matched_gain: verifiedMatchedGain,
    observed_budget_pass: budgetObservedPass,
    ...
    paired_metric_internal_consistency_required: true,
    minimum_matched_pairs: 3,
    hard_invariant_and_negative_transfer_veto_required: true,
    explicit_token_and_latency_budget_binding_required: true,
...
export function verifyRsiDormantSkillRetrievalReview(review, args = {}) {
  ...
  if (
    ...
    || review.paired_metric_internal_consistency_required !== true
    || review.minimum_matched_pairs !== 3
    || review.hard_invariant_and_negative_transfer_veto_required !== true
    || review.explicit_token_and_latency_budget_binding_required !== true
  ...
  const canonical = createRsiDormantSkillRetrievalReview({
    ...
    context_token_budget: review.context_token_budget,
    control_token_cost: review.control_token_cost,
    treatment_token_cost: review.treatment_token_cost,
    latency_budget_ms: review.latency_budget_ms,
    control_latency_ms: review.control_latency_ms,
    treatment_latency_ms: review.treatment_latency_ms,
