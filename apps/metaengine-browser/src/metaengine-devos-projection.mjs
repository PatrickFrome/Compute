export const METAENGINE_DEVOS_PROJECTION_SCHEMA = 'metaengine.devos.projection.v1';
export const METAENGINE_DEVOS_SURFACE_REGISTRY_SCHEMA = 'metaengine.devos.surface-registry.v1';

export const METAENGINE_DEVOS_SURFACE_TYPES = Object.freeze([
  'BROWSER',
  'CODE',
  'TERMINAL',
  'DIFF',
...
function deriveAttention(contexts, system = {}) {
  ...
  if (system.owner_safety_wildcard_disabled === true) {
    rows.push(Object.freeze({
      ...
      severity: 'ERROR',
      title: 'Owner safety wildcard override active',
      reason: 'Safety state requires operator attention',
      ...zeroAuthorityContract(),
    }));
  }
  if (system.supervisor_error) {
    rows.push(Object.freeze({
      ...
      reason: text(system.supervisor_error, 600),
      ...zeroAuthorityContract(),
    }));
  }
  return rows;
  ...
  return safeArray(workbench.contexts).slice(0, maxObjectives).map((context) => {
    ...
    return Object.freeze({
      ...
      progress: Object.freeze({
        ready: Number(context?.progress?.ready || 0),
        active: Number(context?.progress?.active || 0),
        blocked: Number(context?.progress?.blocked || 0),
        completed: Number(context?.progress?.completed || 0),
        failed: Number(context?.progress?.failed || 0),
...
export function createMetaengineDevOSSurfaceRegistry() {
  return Object.freeze({
    schema: METAENGINE_DEVOS_SURFACE_REGISTRY_SCHEMA,
    types: Object.freeze(METAENGINE_DEVOS_SURFACE_TYPES.map((type) => Object.freeze({
      type,
      runtime_bound: type === 'BROWSER',
      default_role: type === 'BROWSER' ? 'EXECUTION_SURFACE' : 'PRESENTATION_SURFACE',
      authority_effect: false,
    }))),
...
export function projectMetaengineDevOS(snapshot = {}, options = {}) {
  ...
  const workspaces = contexts.map((context) => {
    const durable = durableGroups.find((group) =>
      text(group?.context_id, 160) === context.context_id
    ...
    return Object.freeze({
      ...
      branch_name: text(durable?.branch_name, 240),
      base_sha: text(durable?.base_sha, 80),
      state: text(durable?.state, 64)?.toUpperCase() || 'AVAILABLE',
      ...zeroAuthorityContract(),
    });
  });
  ...
  const sessions = contexts.map((context) => Object.freeze({
    ...
    artifact_count: context.artifact_refs.length,
    last_activity_at: context.last_activity_at,
    surface_ids: Object.freeze(browser
      .filter((surface) => surface.session_id === context.session_id)
      .map((surface) => surface.surface_id)),
    ...zeroAuthorityContract(),
  }));

  const supervisorError = snapshot?.supervisor?.last_error || snapshot?.supervisor?.devos_last_error || null;
  ...
  return Object.freeze({
    schema: METAENGINE_DEVOS_PROJECTION_SCHEMA,
    mode: 'DEVELOPMENT_OS',
    primary_object: 'SESSION',
    hierarchy: Object.freeze(['OBJECTIVE', 'WORKSPACE', 'SESSION', 'TASK', 'SURFACE', 'ARTIFACT']),
    objectives: freezeRows(contexts.map((context) => ({
      objective_id: context.objective_id,
      context_id: context.context_id,
      title: context.title,
      status: context.status,
      session_ids: Object.freeze([context.session_id]),
      attention_count: attention.filter((row) => row.objective_id === context.objective_id).length,
      ...zeroAuthorityContract(),
    }))),
    workspaces: freezeRows(workspaces),
    sessions: freezeRows(sessions),
    surfaces: freezeRows(browser),
    artifacts: freezeRows(artifacts.slice(0, 512)),
    attention: freezeRows(attention.slice(0, 256)),
    selected: Object.freeze({
      objective_id: contexts[0]?.objective_id || null,
      workspace_id: workspaces[0]?.workspace_id || null,
      session_id: contexts[0]?.session_id || primarySessionId,
      surface_id: selectedSurface?.surface_id || null,
    }),
    surface_registry: createMetaengineDevOSSurfaceRegistry(),
    counts: Object.freeze({
      objectives: contexts.length,
      workspaces: workspaces.length,
      sessions: sessions.length,
      tasks: sessions.reduce((sum, session) => sum + session.task_count, 0),
      surfaces: browser.length,
      artifacts: Math.min(artifacts.length, 512),
      attention: Math.min(attention.length, 256),
    }),
    bounded: true,
    max_objectives: maxObjectives,
    max_tasks_per_session: maxTasksPerSession,
    max_surfaces: maxSurfaces,
    no_url_heuristic_grouping: true,
    no_title_heuristic_grouping: true,
    browser_is_shell: false,
    browser_is_surface: true,
    ...zeroAuthorityContract(),
  });
