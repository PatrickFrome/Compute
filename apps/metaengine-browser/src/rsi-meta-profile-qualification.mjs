...
export class RsiMetaProfileQualificationLedger{
  ...
  qualificationByDigest(qualification_digest){if(!this.#initialized)throw new Error('rsi_meta_profile_ledger_not_initialized');const d=exactDigest(qualification_digest,'qualification');const row=this.#rows.find(x=>x.qualification_digest===d);return row?Object.freeze(structuredClone(row)):null}
