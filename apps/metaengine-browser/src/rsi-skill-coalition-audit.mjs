...
export class RsiSkillCoalitionAuditStore{
  ...
  async init(){if(this.#init)return this.snapshot();await fs.mkdir(path.dirname(this.#path),{recursive:true});try{const p=JSON.parse(await fs.readFile(this.#path,'utf8'));zero(p,'state');const clone=structuredClone(p);delete clone.state_digest;if(dg(clone)!==digest(p.state_digest,'state'))throw new Error('rsi_coalition_state_digest_mismatch');this.#rows=p.rows.map(verifyRsiSkillCoalitionObservation)}catch(e){if(e?.code!=='ENOENT')throw e}this.#init=true;return this.snapshot()}
  async add(row){if(!this.#init)throw new Error('rsi_coalition_not_initialized');const x=verifyRsiSkillCoalitionObservation(row);if(this.#rows.some(r=>r.observation_digest===x.observation_digest))return Object.freeze({state:'IDEMPOTENT',authority_effect:false});if(this.#rows.length>=MAX_ROWS)throw new Error('rsi_coalition_capacity_exceeded');this.#rows.push(x);const s=state(this.#source,this.#rows);await fs.writeFile(this.#path,`${JSON.stringify(s)}\n`,'utf8');return Object.freeze({state:'APPENDED',authority_effect:false})}
