...
function normalizeSourceRows(rows){
  ...
  const epochSeq=checked[0].entry.evaluation_epoch_seq;
  ...
  for(const row of checked){
    ...
    if(e.evaluator_generation_seq!==generationSeq)throw new Error('rsi_consolidation_cross_generation_sequence_forbidden');
    if(e.evaluator_generation_digest!==generation)throw new Error('rsi_consolidation_cross_generation_forbidden');
    ...
    if(e.evaluation_epoch_seq!==epochSeq)throw new Error('rsi_consolidation_cross_epoch_sequence_forbidden');
    if(e.evaluation_epoch_digest!==epoch)throw new Error('rsi_consolidation_cross_epoch_forbidden');
...
export function verifyRsiKnowledgeConsolidationProposal(proposal,{source_rows}={}){
  ...
  if(proposal.external_consolidator!==true||proposal.external_scope_owner!==true||proposal.authored_by_candidate!==false
    ||proposal.same_evaluator_generation_sequence_required!==true||proposal.same_evaluator_generation_required!==true
    ||proposal.same_generation_history_anchor_required!==true||proposal.same_evaluation_epoch_sequence_required!==true
    ||proposal.same_evaluation_epoch_required!==true||proposal.source_diversity_required!==true||proposal.source_evidence_preserved_by_digest!==true
