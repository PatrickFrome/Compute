...
export class RsiRuntimeSkillLifecycle{
  ...
  async init(){
    ...
    try{
      ...
      if(!Array.isArray(rawExposureHolds)
        ||Number(parsed.admission_exposure_hold_count??rawExposureHolds.length)!==rawExposureHolds.length){
        throw new Error('rsi_runtime_skill_exposure_hold_state_invalid');
      }
      const normalizedExposureHolds=rawExposureHolds.map(value=>exactDigest(value,'exposure_hold_skill'));
      if(new Set(normalizedExposureHolds).size!==normalizedExposureHolds.length)throw new Error('rsi_runtime_skill_exposure_hold_duplicate');
      if(normalizedExposureHolds.length>0&&(parsed.admission_exposure_holds_force_nonactive!==true
        ...
        throw new Error('rsi_runtime_skill_exposure_hold_policy_invalid');
      }
      this.#exposureHolds=new Set(normalizedExposureHolds);
      ...
      if(this.#library){
        for(const held of this.#exposureHolds){
          if(!this.#library.entries.some(entry=>entry.skill_digest===held))throw new Error('rsi_runtime_skill_exposure_hold_unknown_skill');
        }
        createRsiSkillLibraryGovernance({
  ...
  async executePreparedLibraryAdmissionAttempt({
    ...
    const priorExposureHolds=this.#exposureHolds;
    try{
      ...
      if(!added||added.evidence_window_count!==0||added.state!=='DORMANT_CAP'||added.active_for_composition!==false
        ||added.admission_exposure_hold!==true){
      ...
      return zero({
        ...
        admission_exposure_held:true,admission_exposure_hold_skill_digest:row.proposed_skill_digest,
        same_effect_id_retry_allowed:false,pre_effect_readback_passed:true,
      });
    }catch(error){
      this.#library=prior;
      this.#exposureHolds=priorExposureHolds;
      throw error;
    }
  }
  async reconcileLibraryAdmissionAttempt({
    ...
    return zero({
      ...
      admission_exposure_hold_skill_digest:nextState==='CONFIRMED_APPLIED_STORAGE_ONLY'?row.proposed_skill_digest:null,
...
export function rsiRuntimeSkillLifecycleTrustRootSnapshot(){
  const root={
    ...
    admission_exposure_holds_supported:true,storage_admission_does_not_imply_retrieval_exposure:true,
    admission_exposure_hold_release_requires_external_governance:true,
