...
export class NativeSupervisorClient {
  ...
  constructor({ identity, fetchImpl = globalThis.fetch, getState, executeCommand, version, intervalMs = 2000, beforeSelfUpdateInstall = null }) {
    ...
    this.#selfUpdate = new SelfUpdateRuntime({
      ...
      // Phase 1: persist the exact shared handoff receipt while N still owns the singleton.
      ...
      // Phase 2: quiesce command polling, release the singleton, prove release, then permit NSIS launch.
      beforeInstallerLaunch: async () => {
        ...
        if (app.hasSingleInstanceLock()) throw new Error('native_supervisor_self_update_singleton_release_failed');
      },
    });
  }
