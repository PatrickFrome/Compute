...
function l1() {
  const observer = new RsiCommandPlaneLivenessObserver({
    ...
    clock: () => Date.parse('2026-09-17T10:41:40.000Z'),
...
test('L1 mutation contract binds the frozen hypothesis to exactly one non-authority transport seam', () => {
  ...
  assert.deepEqual(contract.allowed_mutations, [{ path: TARGET, change: 'MODIFY' }]);
  assert.deepEqual(contract.immutable_causal_components, [
...
test('targeted builder admits only the exact digest-bound helper mutation', () => {
  ...
  const targeted = prepareRsiTargetedCandidateBuild({
  ...
  assert.deepEqual(targeted.exact_mutation_set, [{ path: TARGET, change: 'MODIFY' }]);
  assert.deepEqual(targeted.generic_build_plan.mutation_manifest, [{ path: TARGET, change: 'MODIFY' }]);
...
test('targeted builder rejects extra, substituted and native-supervisor mutations before generic materialization', () => {
  ...
  for (const mutations of cases) {
    assert.throws(() => prepareRsiTargetedCandidateBuild({
      experiment_plan: plan,
      hypothesis,
      mutation_contract: contract,
      source_snapshot: snapshot,
      mutations,
    }), /rsi_targeted_mutation_set_mismatch/);
  }
});
...
test('targeted builder rejects source snapshots that omit the exact mutation target', () => {
  const { hypothesis, plan, contract, snapshot } = l1();
  const missing = { ...snapshot, source_files: ['apps/metaengine-browser/src/main.mjs'], source_file_count: 1 };
  assert.throws(() => prepareRsiTargetedCandidateBuild({
    experiment_plan: plan,
    hypothesis,
    mutation_contract: contract,
    ...
    mutations: [{ path: TARGET, change: 'MODIFY' }],
  }), /rsi_targeted_source_snapshot_missing_target/);
});
...
test('digest tampering at hypothesis, plan, contract or targeted envelope fails closed', () => {
  ...
  assert.throws(() => prepareRsiTargetedCandidateBuild({
    experiment_plan: badPlan,
    hypothesis,
    mutation_contract: contract,
    source_snapshot: snapshot,
    mutations: [{ path: TARGET, change: 'MODIFY' }],
  }), /experiment_plan_digest_mismatch/);
