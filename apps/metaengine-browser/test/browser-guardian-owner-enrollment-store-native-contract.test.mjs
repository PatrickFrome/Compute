...
test('trusted root is held without delete sharing across create-if-absent', () => {
  ...
  const create = between(
    cpp,
    'OwnerEnrollmentStoreResult OwnerEnrollmentStore::createIfAbsent(',
  ...
  assert.ok(guard >= 0);
  assert.ok(before > guard);
  assert.ok(rename > before);
  assert.ok(failedReadback > rename);
  assert.ok(successReadback > failedReadback);
});
...
test('every failed physical commit barrier is reconciled by fresh durable readback', () => {
  ...
  for (const token of [
    ...
    'OwnerEnrollmentStoreOutcome::NoEffectProven',
    'OWNER_STORE_COMMIT_NO_EFFECT_PROVEN',
    'OwnerEnrollmentStoreOutcome::EffectExact',
    'OWNER_STORE_COMMIT_RESULT_EXACT_AFTER_ERROR',
    'OwnerEnrollmentStoreOutcome::Conflict',
    'OwnerEnrollmentStoreOutcome::Corrupt',
    'OwnerEnrollmentStoreOutcome::Ambiguous',
    'OWNER_STORE_COMMIT_RESULT_AMBIGUOUS',
