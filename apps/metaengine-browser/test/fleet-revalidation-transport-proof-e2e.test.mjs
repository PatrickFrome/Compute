...
function makeHarness() {
  ...
  const observer = () => createFleetTargetLocalObserver({
    lookupView: (tabId) => tabs.has(tabId) ? {
...
test('restart requires fresh local revalidation and exact fresh transport proof before ACTIVE', async () => {
  const h = makeHarness();
  ...
  const revalidated = await restarted.revalidateTargetBinding({
    agent_id: afterRestart.agent_id,
    observeLocalTarget: h.observer(),
  });
