...
test('composition fails on interface mismatch, cycles and missing library skill',()=>{
  ...
  assert.throws(()=>createRsiSkillCompositionPlan({
    ...
    nodes:[
      {node_id:'a',skill_id:analyze.skill_id,skill_version:1,skill_digest:analyze.skill_digest,max_invocations:1},
      {node_id:'r',skill_id:retrieve.skill_id,skill_version:1,skill_digest:retrieve.skill_digest,max_invocations:1},
    ],
    edges:[{from:'r',to:'a'}],
    max_total_context_tokens:8192,
    max_total_output_tokens:2048,
  ...
  assert.throws(()=>createRsiSkillCompositionPlan({
    ...
    nodes:[
      {node_id:'a',skill_id:loopA.skill_id,skill_version:1,skill_digest:loopA.skill_digest,max_invocations:1},
      {node_id:'b',skill_id:loopB.skill_id,skill_version:1,skill_digest:loopB.skill_digest,max_invocations:1},
    ],
...
test('composition budget is hard-bounded before any skill activation',()=>{
  ...
  assert.throws(()=>createRsiSkillCompositionPlan({
    ...
    input_schema_digest:d('1'),
    output_schema_digest:d('2'),
    nodes:[{node_id:'a',skill_id:analyze.skill_id,skill_version:1,skill_digest:analyze.skill_digest,max_invocations:2}],
    edges:[],
...
test('usage receipt is external learning evidence only and helpful outcome requires hard invariants',()=>{
  ...
  const plan=createRsiSkillCompositionPlan({
    ...
    input_schema_digest:d('1'),
    output_schema_digest:d('2'),
    nodes:[{node_id:'a',skill_id:analyze.skill_id,skill_version:1,skill_digest:analyze.skill_digest,max_invocations:1}],
    edges:[],
