...
const binding = { service_name: 'METAENGINEBrowserGuardian', broker_executable: 'METAENGINE Browser Test.exe', expected_owner_sid: ownerSid };
const desired = { state: 'RUNNING', external_stop_requested: false, expected_owner_sid: ownerSid };
const observed = {
...
function sha256Json(value) { return crypto.createHash('sha256').update(JSON.stringify(stable(value))).digest('hex'); }
...
function journal(state = 'INTENT_RECORDED', overrides = {}) {
  ...
  return {
    schema: 'metaengine.browser-guardian.session-broker-effect-journal.v1', version: '1.0.0',
    service_name: binding.service_name, broker_executable: binding.broker_executable, expected_owner_sid: ownerSid,
    state, effect_id: '11111111-2222-4333-8444-555555555555', effect_generation: 1,
    plan, plan_digest: sha256Json(plan), physical_effect_attempted: attempted, effect_barrier_crossed: attempted,
    dispatched_pid: dispatched ? 4242 : null,
    dispatched_process_incarnation_id: dispatched ? 'proc-session-broker-1' : null,
    automatic_retry_allowed: false, browser_authority: false, task_authority: false, scheduler_authority: false,
    page_model_text_authority: false, release_authority: false, session_token_authority: false,
    process_effect_authority: false, authority_effect: false, ...overrides,
  };
}

function assertNoAuthority(row) {
  for (const field of [
    'physical_effect_attempted','wts_execution_allowed','scm_start_allowed','process_effect_allowed',
    'journal_mutation_allowed','automatic_retry_allowed','retry_loop_allowed','second_scheduler_allowed',
    'browser_authority','task_authority','page_model_text_authority','release_authority','session_token_authority','authority_effect',
...
test('exact durable INTENT_RECORDED projects ONE_ATTEMPT_CANDIDATE without WTS authority', () => {
  const result = evaluateGuardianSessionBrokerController({ desired, observed, binding, now_ms: now, journal_snapshot: journal() });
...
test('crossed effect barrier always projects RECONCILE and never another attempt', () => {
  for (const state of ['EFFECT_ATTEMPTED', 'EFFECT_DISPATCHED', 'AMBIGUOUS']) {
    const result = evaluateGuardianSessionBrokerController({ desired, observed, binding, now_ms: now, journal_snapshot: journal(state) });
...
test('confirmed broker journal holds instead of raw start replay', () => {
  const result = evaluateGuardianSessionBrokerController({ desired, observed, binding, now_ms: now, journal_snapshot: journal('CONFIRMED') });
...
test('restart and SCM escalation stay outside controller effect authority', () => {
  const staleObserved = {
    ...observed,
    broker: { pid: 111, process_incarnation_id: 'old-broker', session_id: 3, user_sid: ownerSid, heartbeat_at_ms: now - 60_000 },
  ...
  const escalation = evaluateGuardianSessionBrokerController({
    desired, observed: { ...staleObserved, broker_restart_history_ms: [now - 1_000, now - 2_000, now - 3_000] }, binding, now_ms: now,
  });
...
test('controller executable source has no WTS/SCM/process/timer/retry primitive', async () => {
  ...
  for (const forbidden of [
    /CreateProcessAsUserW/i, /WTSQueryUserToken/i, /StartServiceW/i, /Start-Process/i, /child_process/i,
    /\bspawn\s*\(/i, /\bexec(?:File)?\s*\(/i, /setInterval\s*\(/i, /setTimeout\s*\(/i,
    /\bretry\s*\(/i,
  ]) assert.equal(forbidden.test(executable), false, `forbidden controller primitive: ${forbidden}`);
});
