...
test('warm reconcile creates only fleet-owned bound-unverified agents', async () => {
  const h = harness();
  await h.provisioner.init();
  const snap = await h.provisioner.reconcile({ active: false });
  assert.equal(snap.agents.length, 2);
  assert.equal(snap.counts.BOUND_UNVERIFIED, 2);
  assert.ok(snap.agents.every((a) => a.ownership === 'FLEET_OWNED'));
  assert.ok(snap.agents.every((a) => a.lifecycle_state !== 'READY'));
  assert.equal(h.loads.length, 2);
});

test('active reconcile scales to desired fleet and is idempotent', async () => {
  const h = harness();
  await h.provisioner.init();
...
test('ambiguous tab creation is never automatically retried', async () => {
  ...
  assert.equal(first.counts.PROVISIONING_AMBIGUOUS, 2);
  await h.provisioner.reconcile({ active: false });
  const second = h.provisioner.snapshot();
  assert.equal(second.counts.PROVISIONING_AMBIGUOUS, 2);
  assert.ok(second.agents.every((a) => a.automatic_retry_allowed === false));
});

test('tab loss preserves agent id and recovery increments conversation epoch', async () => {
  const h = harness();
  await h.provisioner.init();
  await h.provisioner.reconcile({ active: false });
  ...
  await h.provisioner.reconcile({ active: false });
  const after = h.provisioner.snapshot().agents.find((a) => a.agent_id === before.agent_id);
...
test('pre-actuation state is persisted before createTab effect', async () => {
  ...
  await p.init();
  await p.reconcile({ active: false });
