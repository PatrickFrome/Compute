...
function gateway(overrides = {}) {
  const calls = { context: [], batch: [], delta: [], emergency: [] };
...
test('gateway exposes exactly four goal-level tools and no raw execution surface', () => {
  assert.deepEqual(FAST_CONTROL_TOOL_NAMES, ['context_get', 'run_submit', 'run_status', 'emergency_stop']);
...
test('unknown tools and unknown fields fail closed before delegated calls', async () => {
  ...
  assert.equal(calls.context.length + calls.batch.length + calls.delta.length + calls.emergency.length, 0);
});
