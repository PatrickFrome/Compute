...
test('DevOS projection exposes objective/workspace/session/task/surface/artifact hierarchy without creating authority', () => {
  ...
  const taskSession = projection.sessions.find((row) => row.session_id === 'session:ctx.release');
  const browserSession = projection.sessions.find((row) => row.session_id === 'session:browser-unbound');
  assert.equal(taskSession.tasks[0].status, 'ACTIVE');
  ...
  assert.equal(browserSession.authority_effect, false);
  ...
  assert.equal(projection.selected.session_id, 'session:browser-unbound');
...
test('DevOS projection is bounded and does not infer workspace binding from URL or title', () => {
  ...
  const taskSession = projection.sessions.find((row) => row.session_id === 'session:ctx.release');
  assert.equal(taskSession.tasks.length, 1);
...
test('invalid collaboration workbench fails closed instead of synthesizing DevOS objectives', () => {
  ...
  assert.equal(projection.workspaces.length, 0);
  assert.equal(projection.sessions.length, 1);
  assert.equal(projection.sessions[0].session_id, 'session:browser-unbound');
  assert.equal(projection.sessions[0].browser_only, true);
  assert.equal(projection.sessions[0].task_count, 0);
  assert.equal(projection.sessions[0].objective_id, null);
  assert.equal(projection.sessions[0].workspace_id, null);
  assert.equal(projection.sessions[0].authority_effect, false);
  assert.equal(projection.surfaces.length, 2);
  assert.equal(projection.surfaces.every((surface) => surface.session_id === 'session:browser-unbound'), true);
  assert.equal(projection.authority_effect, false);
});
