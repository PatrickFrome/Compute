...
const commands = Array.from({ length: WIDTH }, (_, index) => ({
  ...
  action: 'TYPED_CLICK',
  payload: {
    tab_id: `tab_00000000-0000-4000-8000-${String(index + 1).padStart(12, '0')}`,
    role: 'button',
...
test('adaptive runtime reaches full mutation capacity through the coordinator default BrowserCell path', async () => {
  const scheduler = new NativeSupervisorCommandLaneScheduler({ readConcurrency: 128, mutationConcurrency: WIDTH });
  const started = [];
  ...
  assert.deepEqual(started, commands.map((command) => [command.command_id, command.payload.tab_id]));

  for (let index = WIDTH - 1; index >= 0; index -= 1) releases.get(commands[index].command_id)();
