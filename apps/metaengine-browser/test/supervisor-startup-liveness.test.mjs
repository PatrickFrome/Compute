...
test('Browser runtime registers early but cannot start before host resilience bootstrap and watchdog fencing', async () => {
  ...
  assert.match(main, /STARTUP_RETRY_BASE_MS/);
  assert.match(main, /STARTUP_RETRY_MAX_MS/);
  assert.match(main, /Math\.min\(STARTUP_RETRY_MAX_MS/);
  assert.match(main, /startupRetryTimer\.unref\?\.\(\)/);
});
