...
async function main() {
  ...
  try {
    ...
    sessionA = session.fromPartition(partitionA, { cache: false });
    sessionB = session.fromPartition(partitionB, { cache: false });
    ...
    await Promise.all([sessionA, sessionB].filter(Boolean).map((value) => value.clearStorageData().catch(() => {})));
