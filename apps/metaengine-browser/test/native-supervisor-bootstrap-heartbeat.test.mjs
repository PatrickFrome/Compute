...
test('bootstrap heartbeat canonicalizes stale authority to always-on CONTROL without command authority', () => {
  ...
  assert.equal(payload.state.operator_mode, 'CONTROL');
...
test('bootstrap pump posts only signed liveness heartbeat and never leases commands while continuity start is pending', async () => {
  const calls = [];
  const identity = {
  ...
  const result = await sendBootstrapHeartbeat({
    identity,
    fetchImpl,
    getState: async () => ({ supervisor_mode: 'MONITOR', armed: false, operator_mode: 'OBSERVE', tabs: [] }),
    version: '0.6.3-dev.test',
    startedAt: '2026-08-31T06:00:00.000Z',
  });
  ...
  assert.equal(calls.find((row) => row.kind === 'headers').path, `${NATIVE_SUPERVISOR_RUNTIME_PATH}/v1/heartbeat`);
