...
test('clean genesis quarantines stale runtime topology but preserves auth identity and browser session data', async () => {
  ...
  try {
    ...
    assert.equal(result.supervisor_mode, 'OFF');
    ...
    assert.equal(control.supervisor_mode, 'OFF');
    assert.equal(control.armed, false);
...
test('clean genesis is exactly-once for a runtime generation', async () => {
