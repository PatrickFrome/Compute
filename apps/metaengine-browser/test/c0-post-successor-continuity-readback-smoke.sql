...
create or replace function public.h205f22_compute_unified_verified_restart_successor_continuity_v1(
 ...
 if p_attempt_id='bad' then return jsonb_build_object('continuity_accepted',false,'automatic_retry_allowed',false,'restart_authorized',false,'wake_replay_authorized',false,'lease_mutation_authorized',false,'authority_effect',false); end if;
...
insert into public.compute_fabric_a2_browser_supervisor_state_h205f22 values(
 'client-new','11111111-1111-1111-1111-111111111111',statement_timestamp(),'CONTROL',true,
 jsonb_build_object('perception',jsonb_build_object('process_incarnation_id','proc-new'),'supervisor_lifecycle',jsonb_build_object(
...
insert into public.compute_fabric_a2_browser_architecture_checkpoint_h205f22 values('CP','AUTHORITATIVE','integration/compute-unified-v1',repeat('a',40),statement_timestamp());
...
DO $$ declare w uuid:='11111111-1111-1111-1111-111111111111'; r jsonb; begin
 ...
 if not coalesce((r->>'verified')::boolean,false) or r->>'reason' is distinct from 'POST_SUCCESSOR_CONTINUITY_VERIFIED' or coalesce((r->>'automatic_retry_allowed')::boolean,true) or coalesce((r->>'restart_authorized')::boolean,true) or coalesce((r->>'wake_replay_authorized')::boolean,true) or coalesce((r->>'lease_mutation_authorized')::boolean,true) or coalesce((r->>'authority_effect')::boolean,true) then raise exception 'valid readback rejected/leaked authority: %',r; end if;

 update public.compute_fabric_a2_browser_supervisor_state_h205f22 set last_seen_at=statement_timestamp()-interval '5 minutes' where client_id='client-new';
 r:=public.h205f22_compute_unified_post_successor_continuity_readback_v1(w,'ok','restart:ok',repeat('1',64),42,'client-new','proc-new',8,repeat('a',40),interval '120 seconds');
 ...
 update public.compute_fabric_a2_browser_supervisor_state_h205f22 set last_seen_at=statement_timestamp() where client_id='client-new';
