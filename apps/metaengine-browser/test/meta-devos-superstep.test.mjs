...
function inputs(){return{schema:'metaengine.meta-orchestrator.authoritative-inputs.v1',workspace_id:workspaceId,roadmap_id:roadmapId,roadmap_authority:authority,plan_state:planState(),tasks:[],roadmap_receipts:[],task_meta_projection_only:true,task_payload_exposed:false,result_summary_exposed:false,scheduler_identity_exposed:false,receipt_summary_exposed:false,receipt_evidence_exposed:false,automatic_retry_allowed:false,task_content_authority:false,scheduler_authority:false,browser_authority:false,release_authority:false,authority_effect:false}}
function capacity(slots){return{schema:'metaengine.devos.scheduler-capacity.v1',workspace_id:workspaceId,state:'FRESH',source:'DEVOS_SCHEDULER_SNAPSHOT',available_slots:slots,by_role:{IMPLEMENTER:slots},observed_at:'2026-09-01T07:55:00Z',freshness_horizon_seconds:45,transport_admission:'ACTIVE_EXACT_PROOF_V1',scheduler_source:'NATIVE_SUPERVISOR_HEARTBEAT',scheduler_policy:'IDLE_ROLE_FAIR_SHARE_V1',automatic_retry_allowed:false,authority_effect:false}}
...
function runner({slots=3,leaderRow=leader(),frontierError=null,inputsError=null}={}){
  const calls=[];
  const rpc=async(name,args)=>{
    ...
    if(name==='devos_fleet_capacity_snapshot_v1')return capacity(slots);
...
test('leader migrations are RLS fenced, epoch-bound, and never allocate DevOS leases',async()=>{
