...
test('worker owns one worktree-aware bounded repo search index and no caller-selected path capability', async () => {
  const source = await readFile(new URL('../src/development-plane-worker.cjs', import.meta.url), 'utf8');
  assert.match(source, /const repoSearchIndex = new WorktreeAwareDevOSRepoSearchIndex\(\{/);
  assert.match(source, /METAENGINE_DEVOS_WORKTREE_WATCH/);
  assert.match(source, /const repoSourceTracker = new RepoSourceTracker\(\{/);
  ...
  assert.match(source, /devos_repo_search_cache: 'HEAD_PLUS_WORKTREE_EVENT_EPOCH'/);
