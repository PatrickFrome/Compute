...
test('worker cannot relaunch before exact old parent absence and never retries ambiguous termination blindly',async()=>{
  ...
  const retryLoop=source.match(/async function relaunchUntilResolved\(state, journal\) \{[\s\S]*?\n\}/)?.[0] || '';
  const mainRelaunch=source.match(/if \(parentAlive\(\)\) return;\s*await relaunchUntilResolved\(state, journal\);/)?.[0] || '';
  assert.ok(beginTermination>=0&&kill>beginTermination);
  assert.ok(beginRelaunch>=0&&spawn>beginRelaunch);
  assert.match(retryLoop,/parentAlive\(\)\) return;/);
  assert.match(retryLoop,/const outcome = await relaunchOnce\(state, journal\)/);
  assert.match(retryLoop,/retryAllowedForOutcome\(outcome\) && journal\.relaunchRetryAllowed\(\)/);
  assert.match(mainRelaunch,/if \(parentAlive\(\)\) return;/);
  assert.match(mainRelaunch,/await relaunchUntilResolved\(state, journal\)/);
  ...
  assert.match(source,/Ambiguous effects are terminal for actuation, not for evidence: never replay/);
