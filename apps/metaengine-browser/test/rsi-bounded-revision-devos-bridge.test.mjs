...
function phase31SourceRows(prefix='phase31', {state='SUPPORTED_FOR_BOUNDED_REVISION',generationSeq=1,epochSeq=1} = {}) {
  const shared={
    evaluator_root_digest:labelDigest(`${prefix}-evaluator-root`),
    evaluator_generation_digest:labelDigest(`${prefix}-generation`),
    evaluator_generation_history_anchor_digest:labelDigest(`${prefix}-history-anchor`),
  ...
  const a=phase30OutcomeEvidence(`${prefix}-a`,{state,sharedEvaluation:shared,generationSeq,epochSeq});
  const b=phase30OutcomeEvidence(`${prefix}-b`,{state,sharedEvaluation:shared,generationSeq,epochSeq});
...
test('Phase31 consolidates only diverse same-generation same-kind Phase30 evidence and remains zero-authority',()=>{
  ...
  assert.equal(proposal.evaluator_generation_seq,1);
  assert.equal(proposal.evaluation_epoch_seq,1);
...
test('Phase31 refuses consolidation across exact evaluator generation or epoch sequences even when semantic class matches',()=>{
  const g1=phase31SourceRows('sequence-fence',{generationSeq:1,epochSeq:1});
  const g2=phase31SourceRows('sequence-fence',{generationSeq:2,epochSeq:1});
  assert.throws(
    ()=>phase31Proposal([g1[0],g2[1]],'sequence-generation-mix'),
    /cross_generation_sequence_forbidden/,
  );

  const e2=phase31SourceRows('sequence-fence',{generationSeq:1,epochSeq:2});
  assert.throws(
    ()=>phase31Proposal([g1[0],e2[1]],'sequence-epoch-mix'),
    /cross_epoch_sequence_forbidden/,
  );

  const anchorDrift=structuredClone(g1[1]);
  anchorDrift.entry.evaluator_generation_history_anchor_digest=labelDigest('phase31-forged-history-anchor');
  const core=structuredClone(anchorDrift.entry);delete core.entry_digest;
  anchorDrift.entry.entry_digest=dg(core);
  assert.throws(
    ()=>phase31Proposal([g1[0],anchorDrift],'sequence-anchor-mix'),
    /entry_digest_mismatch|generation_history_anchor_mismatch/,
  );
});
