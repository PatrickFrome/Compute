...
function phase32ExactSkillEvidenceReviewArgs(fx,label='phase32-exact-skill-evidence',overrides={}){
  return {
    ...
    treatment_receipt_digest:labelDigest(`${label}-treatment`),
    local_evidence_digest:labelDigest(`${label}-local-evidence`),
    behavioral_abstraction_digest:labelDigest(`${label}-behavioral-abstraction`),
    behavioral_invariant_set_digest:labelDigest(`${label}-behavioral-invariants`),
    failure_attribution_digest:labelDigest(`${label}-failure-attribution`),
    attempt_count:12,
    success_count:11,
...
function phase34bLifecycleFixture(label='phase34b-lifecycle'){
  ...
  const skill=fx.p33.skillFx.skill;
  const skillEvidence=fx.p33.skillEvidenceReview.standard_skill_evidence;
  ...
  return {
    fx,certificate,currentLibrary,skill,skillEvidence,successorLibrary,
    effectId:labelDigest(label+'-effect-id'),
    idempotencyKey:labelDigest(label+'-idempotency-key'),
    executorIdentity:labelDigest(label+'-effect-executor'),
...
test('Phase34B runtime service closes direct-adopt bypass and routes storage append through the durable one-attempt lifecycle',async(t)=>{
  ...
  // Fresh post-append consumer-local credit is a separate evidence transition.
  // It is the first point at which releasing the hold can predict EXPLORATION_ACTIVE.
  ...
  assert.notEqual(freshProvenance.provenance_digest,admissionProvenance.provenance_digest);
  ...
  assert.equal(positivePreview.current_admission_exposure_hold,true);
  assert.equal(positivePreview.next_admission_exposure_hold,false);
  ...
  const freshExposureReview=await runtime.createSkillExposureReleaseReview({
    review_id:'phase36.runtime.exposure-review.fresh-credit.1',
    skill_digest:fx.skill.skill_digest,
    consumer_model_family:'GPT_5_6_SOL',
    environment_fingerprint:'env.phase36.post-append.consumer',
    task_signature_digest:labelDigest('phase36-post-append-task'),
    routing_context_manifest_digest:labelDigest('phase36-fresh-routing-context'),
    retrieval_profile_digest:labelDigest('phase36-fresh-retrieval-profile'),
    memory_context_digest:labelDigest('phase36-fresh-memory-context'),
    harness_integrity_digest:labelDigest('phase36-fresh-harness-integrity'),
    benchmark_provenance_digest:labelDigest('phase36-fresh-benchmark-provenance'),
    matched_comparison_receipt_digest:labelDigest('phase36-fresh-matched-comparison'),
    negative_transfer_receipt_digest:labelDigest('phase36-fresh-negative-transfer'),
    cost_latency_receipt_digest:labelDigest('phase36-fresh-cost-latency'),
    source_grounding_receipt_digest:labelDigest('phase36-fresh-source-grounding'),
    external_policy_digest:labelDigest('phase36-fresh-policy'),
    matched_pair_count:4,
    skill_success_count:4,
    reference_success_count:2,
    repair_count:1,
    ...
    memory_safety_pass:true,
    source_grounding_pass:true,
    governance_reviewer_identity_digest:labelDigest('phase36-fresh-governance-reviewer'),
    matched_evaluator_identity_digest:labelDigest('phase36-fresh-matched-evaluator'),
    security_reviewer_identity_digest:labelDigest('phase36-fresh-security-reviewer'),
    external_governance_reviewer:true,
    external_matched_evaluator:true,
    external_security_reviewer:true,
    authored_by_candidate:false,
  ...
  const certificate=await runtime.createSkillExposureReleaseCertificate({
    ...
    bounded_canary_policy_digest:labelDigest('phase36-certificate-canary-policy'),
    bounded_canary_result_digest:labelDigest('phase36-certificate-canary-result'),
    ...
    external_release_certifier_identity_digest:labelDigest('phase36-release-certifier'),
    external_shadow_evaluator_identity_digest:labelDigest('phase36-release-shadow-evaluator'),
    external_canary_evaluator_identity_digest:labelDigest('phase36-release-canary-evaluator'),
  ...
  assert.equal(preparedRelease.retrieval_exposure_changed,false);
  assert.equal(preparedRelease.full_activation_authorized,false);
  assert.equal(runtime.snapshot().runtime_skill_lifecycle.exposure_release_attempt_count,1);
  assert.equal(runtime.snapshot().runtime_skill_lifecycle.exposure_release_attempt_state_counts.PREPARED,1);
  assert.equal(runtime.snapshot().runtime_skill_lifecycle.admission_exposure_hold_count,1);
  assert.throws(()=>runtime.createSkillActivationView([fx.skill.skill_digest]),/requested_skill_not_active:DORMANT_CAP/);
  assert.equal(runtime.snapshot().ledger.last_event_type,'SKILL_EXPOSURE_RELEASE_ATTEMPT_PREPARED');

  const preparedReadback=runtime.skillExposureReleaseAttemptSnapshot('phase36.runtime.release-attempt.1');
  assert.equal(preparedReadback.current_state,'PREPARED');
  assert.equal(preparedReadback.release_certificate_digest,certificate.certificate_digest);
  ...
  // ATTEMPTED is a durable fence, but its process-local effect handoff intentionally
  // does not survive restart. A restarted process must reconcile by readback and may
  // not blindly continue the old effect attempt.
  const restartedRuntime=new RsiRuntimeService({
    source_sha:SOURCE,
    ledgerPath:path.join(dir,'runtime.jsonl'),
    clock:()=>1_800_000_000_100,
  });
  await restartedRuntime.start();
  assert.equal(restartedRuntime.skillExposureReleaseAttemptSnapshot('phase36.runtime.release-attempt.1').current_state,'ATTEMPTED');
  await assert.rejects(()=>restartedRuntime.executeSkillExposureReleaseEffect({
    attempt_id:'phase36.runtime.release-attempt.1',
    effect_executor_identity_digest:labelDigest('phase36-release-effect-executor'),
    external_effect_executor:true,
    authored_by_candidate:false,
  }),/effect_handoff_missing_reconcile_required/);
  assert.equal(restartedRuntime.snapshot().runtime_skill_lifecycle.admission_exposure_hold_count,1);
  const noEffect=await restartedRuntime.reconcileSkillExposureReleaseAttempt({
    attempt_id:'phase36.runtime.release-attempt.1',
    readback_owner_identity_digest:labelDigest('phase36-release-readback-owner'),
    external_readback_owner:true,
    authored_by_candidate:false,
  });
  assert.equal(noEffect.state,'CONFIRMED_NOT_RELEASED_NEW_ATTEMPT_REQUIRED');
  assert.equal(noEffect.additional_effect_attempt_performed,false);
  assert.equal(noEffect.same_effect_id_retry_allowed,false);
  assert.equal(noEffect.new_attempt_required,true);
  assert.equal(noEffect.retrieval_exposure_changed,false);
  assert.equal(restartedRuntime.snapshot().runtime_skill_lifecycle.admission_exposure_hold_count,1);
  assert.equal(restartedRuntime.snapshot().ledger.last_event_type,'SKILL_EXPOSURE_RELEASE_RECONCILED');

  const certificate2=await restartedRuntime.createSkillExposureReleaseCertificate({
  ...
  assert.equal(certificate2.state,'ELIGIBLE_FOR_ONE_ATTEMPT_EXPOSURE_RELEASE');
  assert.notEqual(certificate2.certificate_digest,certificate.certificate_digest);

  const preparedRelease2=await restartedRuntime.prepareSkillExposureReleaseAttempt({
    attempt_id:'phase36.runtime.release-attempt.2',
    certificate:certificate2,
    certificate_args:{release_review:freshExposureReview},
    effect_id_digest:labelDigest('phase36-release-effect-id-2'),
    idempotency_key_digest:labelDigest('phase36-release-idempotency-key-2'),
    effect_executor_identity_digest:labelDigest('phase36-release-effect-executor-2'),
    external_effect_executor:true,
    authored_by_candidate:false,
  });
  assert.equal(preparedRelease2.state,'PREPARED');
  const attemptedRelease2=await restartedRuntime.recordSkillExposureReleaseAttempted({
    attempt_id:'phase36.runtime.release-attempt.2',
    effect_executor_identity_digest:labelDigest('phase36-release-effect-executor-2'),
    external_effect_executor:true,
    authored_by_candidate:false,
  });
  assert.equal(attemptedRelease2.state,'ATTEMPTED');
  assert.equal(attemptedRelease2.effect_handoff_process_bound,true);
  assert.equal(attemptedRelease2.effect_handoff_survives_restart,false);

  const released=await restartedRuntime.executeSkillExposureReleaseEffect({
    attempt_id:'phase36.runtime.release-attempt.2',
    effect_executor_identity_digest:labelDigest('phase36-release-effect-executor-2'),
    external_effect_executor:true,
    authored_by_candidate:false,
  ...
  assert.equal(restartedRuntime.snapshot().runtime_skill_lifecycle.admission_exposure_hold_count,0);
  assert.equal(restartedRuntime.snapshot().runtime_skill_lifecycle.exposure_release_attempt_state_counts.CONFIRMED_RELEASED_EXPLORATION_ONLY,1);
  assert.equal(restartedRuntime.snapshot().runtime_skill_lifecycle.exposure_release_attempt_state_counts.CONFIRMED_NOT_RELEASED_NEW_ATTEMPT_REQUIRED,1);
  const releasedRow=restartedRuntime.verifiedSkillStateReadback().governance.entries.find(row=>row.skill_digest===fx.skill.skill_digest);
  ...
  const explorationView=restartedRuntime.createSkillActivationView([fx.skill.skill_digest]);
  assert.equal(explorationView.selected_count,1);
  assert.equal(explorationView.selected[0].skill_digest,fx.skill.skill_digest);
  assert.equal(restartedRuntime.snapshot().ledger.last_event_type,'SKILL_EXPOSURE_RELEASE_EFFECT_RESULT');
  await assert.rejects(()=>restartedRuntime.executeSkillExposureReleaseEffect({
    attempt_id:'phase36.runtime.release-attempt.2',
    effect_executor_identity_digest:labelDigest('phase36-release-effect-executor-2'),
    external_effect_executor:true,
  ...
  await assert.rejects(()=>restartedRuntime.adoptVerifiedSkillLibrary({
    library:fx.successorLibrary,
    external_library_owner:true,
    authored_by_candidate:false,
  }),/direct_library_adopt_phase34b_required/);
  assert.equal(restartedRuntime.anytimeLibraryAdmissionAttemptSnapshot('phase34b.runtime.attempt.1').current_state,'CONFIRMED_APPLIED_STORAGE_ONLY');
});
...
test('Phase34B lifecycle prepares admission durably before effect, survives restart, and appends storage-only once',async(t)=>{
  ...
  const fx=phase34bLifecycleFixture('prepare-execute');
  ...
  const prepared=await store.prepareLibraryAdmissionAttempt({
  ...
  assert.equal(prepared.state,'PREPARED');
  assert.equal(store.snapshot().library_digest,before.library_digest);
  assert.equal(store.snapshot().library_entry_count,before.library_entry_count);
  assert.equal(store.snapshot().admission_attempt_count,1);
  assert.equal(store.admissionAttemptSnapshot('phase34b.attempt.prepare-execute').current_state,'PREPARED');

  const restored=new RsiRuntimeSkillLifecycle({statePath,source_sha:SOURCE});
  await restored.init();
  assert.equal(restored.admissionAttemptSnapshot('phase34b.attempt.prepare-execute').current_state,'PREPARED');
  assert.equal(restored.verifiedLibrarySnapshot().library_digest,fx.currentLibrary.library_digest);

  const result=await restored.executePreparedLibraryAdmissionAttempt({
    attempt_id:'phase34b.attempt.prepare-execute',
    effect_executor_identity_digest:fx.executorIdentity,
    external_effect_executor:true,
    authored_by_candidate:false,
...
test('Phase34B attempted-but-unapplied admission reconciles from exact predecessor readback without a second effect',async(t)=>{
  const dir=await fs.mkdtemp(path.join(os.tmpdir(),'rsi-phase34b-not-applied-'));
  t.after(()=>fs.rm(dir,{recursive:true,force:true}));
  const statePath=path.join(dir,'skill-lifecycle.json');
  const fx=phase34bLifecycleFixture('not-applied');
  const store=new RsiRuntimeSkillLifecycle({statePath,source_sha:SOURCE});
  await store.init();
  ...
  await store.prepareLibraryAdmissionAttempt({
    attempt_id:'phase34b.attempt.not-applied',
  ...
  const attempted=await store.recordLibraryAdmissionAttempted({
    attempt_id:'phase34b.attempt.not-applied',effect_executor_identity_digest:fx.executorIdentity,
  ...
  const restored=new RsiRuntimeSkillLifecycle({statePath,source_sha:SOURCE});
  await restored.init();
  assert.equal(restored.admissionAttemptSnapshot('phase34b.attempt.not-applied').current_state,'ATTEMPTED');
  const reconciled=await restored.reconcileLibraryAdmissionAttempt({
    attempt_id:'phase34b.attempt.not-applied',
...
test('Phase34B ambiguous attempted admission can reconcile an externally observed successor without re-effect',async(t)=>{
  ...
  t.after(()=>fs.rm(dir,{recursive:true,force:true}));
  const statePath=path.join(dir,'skill-lifecycle.json');
  const fx=phase34bLifecycleFixture('reconcile-applied');
  const store=new RsiRuntimeSkillLifecycle({statePath,source_sha:SOURCE});
  await store.init();
  await store.adoptVerifiedLibrary({library:fx.currentLibrary,external_library_owner:true,authored_by_candidate:false});
  await store.prepareLibraryAdmissionAttempt({
  ...
  await store.recordLibraryAdmissionAttempted({
    attempt_id:'phase34b.attempt.reconcile-applied',effect_executor_identity_digest:fx.executorIdentity,
    external_effect_executor:true,authored_by_candidate:false,
  });

  // Temporary fixture simulates an effect that happened after the durable ATTEMPTED marker.
  ...
  assert.equal(externalEffect.state,'ADOPTED');
  ...
  assert.equal(reconciled.state,'CONFIRMED_APPLIED_STORAGE_ONLY');
  assert.equal(reconciled.additional_effect_attempt_performed,false);
  assert.equal(reconciled.same_effect_id_retry_allowed,false);
...
test('Phase34B preparation fail-closes on stale predecessor and duplicate effect identities before mutation',async(t)=>{
  const dir=await fs.mkdtemp(path.join(os.tmpdir(),'rsi-phase34b-cas-guard-'));
  t.after(()=>fs.rm(dir,{recursive:true,force:true}));
  const statePath=path.join(dir,'skill-lifecycle.json');
  const fx=phase34bLifecycleFixture('cas-guard');
  ...
  assert.equal(store.snapshot().admission_attempt_count,0);

  const exactStore=new RsiRuntimeSkillLifecycle({statePath:path.join(dir,'exact.json'),source_sha:SOURCE});
  await exactStore.init();
  await exactStore.adoptVerifiedLibrary({library:fx.currentLibrary,external_library_owner:true,authored_by_candidate:false});
  await exactStore.prepareLibraryAdmissionAttempt({
    attempt_id:'phase34b.attempt.first',
    admission_certificate:fx.certificate,admission_certificate_args:fx.fx.certificateArgs,
    successor_library:fx.successorLibrary,effect_id_digest:fx.effectId,idempotency_key_digest:fx.idempotencyKey,
    effect_executor_identity_digest:fx.executorIdentity,external_library_owner:true,external_effect_executor:true,authored_by_candidate:false,
  });
  await assert.rejects(()=>exactStore.prepareLibraryAdmissionAttempt({
    attempt_id:'phase34b.attempt.conflict',
    admission_certificate:fx.certificate,admission_certificate_args:fx.fx.certificateArgs,
    successor_library:fx.successorLibrary,effect_id_digest:fx.effectId,
    idempotency_key_digest:labelDigest('phase34b-conflict-idempotency'),
    effect_executor_identity_digest:fx.executorIdentity,external_library_owner:true,external_effect_executor:true,authored_by_candidate:false,
...
test('Phase34B runtime lifecycle trust root freezes CAS, durable pre-effect state and readback-only recovery',()=>{
  ...
  assert.equal(root.admission_attempt_state_is_append_only,true);
