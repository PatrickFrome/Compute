...
function fixture(){
  ...
  const reviewCore={
    ...
    skill_digest:capsule.skill_digest,
    retrieval_reviewer_identity_digest:d('8'),
    consumer_evaluator_identity_digest:d('9'),
    contamination_auditor_identity_digest:d('a'),
    coalition_auditor_identity_digest:d('b'),
    capacity_policy_owner_identity_digest:d('c'),
    admission_effect_executor_identity_digest:d('d'),
    admission_effect_executor_identity_bound:true,
    cross_stage_release_identity_separation_required:true,
...
test('Phase36 certificate rejects identity reuse across R7 review, storage executor and R8 certificate',()=>{
  const fx=fixture();
  assert.throws(()=>createRsiSkillExposureReleaseCertificate(args(fx,{
  ...
  assert.throws(()=>createRsiSkillExposureReleaseCertificate(args(fx,{
    external_canary_evaluator_identity_digest:fx.dormantRetrievalReview.admission_effect_executor_identity_digest,
  })),/cross_stage_identity_separation_required/);
});
...
test('Phase36 exposure-release trust root requires external, bounded, exploration-only evidence and grants no effect authority',()=>{
  ...
  assert.equal(root.release_certificate_cannot_reuse_admission_effect_executor,true);
