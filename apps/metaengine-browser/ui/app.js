...
function developmentPlaneStatus(next) {
  ...
  return { value: compact(state, 10), tone, title: `Development Plane · ${state}${dev.version ? ` · ${dev.version}` : ''}` };
}
...
function selectedTab(next) {
  ...
  return tabs.find((tab) => tab.tab_id === tabState.selected_tab_id) || null;
}
...
async function setLayout(patch = {}) {
  ...
  await api.command('SHELL_LAYOUT_SET', next);
}
...
async function toggleOperations(force = null) {
  const next = force || (requestedLayout.operations === 'OPEN' ? 'CLOSED' : 'OPEN');
  await setLayout({ operations: next });
}
...
function renderActive(next) {
  ...
  activeMeta.textContent = agent
    ? `${text(agent.role, 'AGENT')} · ${text(agent.lifecycle_state, 'UNKNOWN')} · g${Number(agent.generation_epoch || 0)}`
    : hostFor(tab.url);
...
function makeTabRow(tab, active, agent) {
  ...
  button.className = `verticalTab ${active ? 'active' : ''} ${agent ? 'agent' : ''}`;
  button.type = 'button';
  button.setAttribute('role', 'listitem');
  button.title = tab.url || tab.title || 'Tab';
  button.onclick = () => api.command('SELECT_TAB', { tab_id: tab.tab_id }).catch(() => {});
  ...
  button.append(avatar, copy, close);
  return button;
}
...
function section(title, subtitle = '') {
  ...
  const head = el('div', 'opsSectionTitle');
  head.append(el('strong', '', title), el('span', '', subtitle));
...
function renderOverview(next) {
  ...
  const update = updateStatus(next);
  grid.append(
    ...
    metric('Self update', update.value, update.tone),
  );
...
function renderFleet(next) {
  ...
  fragment.append(grid);
  const listSection = section('Agents', `${Array.isArray(fleet.agents) ? fleet.agents.length : 0} registered`);
  listSection.list.className = 'entityList';
  for (const agent of fleet.agents || []) {
    const proof = agent.transport_proof;
    listSection.list.append(entityRow(
  ...
  fragment.append(listSection.wrap);
  return fragment;
}
...
function renderSupervisor(next) {
  ...
  core.list.append(
    kvRow('Mode', sup.supervisor_mode || 'UNKNOWN', stateTone(sup.supervisor_mode)),
    kvRow('Armed', sup.armed === true ? 'TRUE' : 'FALSE', sup.armed === true ? 'good' : 'warn'),
    kvRow('Enrollment', sup.enrollment_status || 'UNKNOWN', stateTone(sup.enrollment_status)),
    kvRow('Heartbeat', sup.last_heartbeat_at || 'UNKNOWN', sup.last_heartbeat_at ? 'good' : 'neutral'),
    kvRow('Last command', sup.last_command_status || 'NONE', stateTone(sup.last_command_status)),
    kvRow('Error', sup.last_error || sup.devos_last_error || 'NONE', sup.last_error || sup.devos_last_error ? 'bad' : 'good'),
  );
  fragment.append(core.wrap);

  const watchdog = sup.bootstrap_heartbeat;
  const watchdogSection = section('Heartbeat watchdog', 'no command leasing');
  watchdogSection.list.append(
  ...
  fragment.append(watchdogSection.wrap);

  const mesh = sup.supervisor_mesh;
  const meshSection = section('Supervisor mesh', sup.supervisor_mesh_wire_projection || 'projection unknown');
  meshSection.list.append(
    kvRow('State', mesh?.state || mesh?.lifecycle_state || (mesh ? 'EXPOSED' : 'UNKNOWN'), mesh ? 'good' : 'neutral'),
    kvRow('Actuation authority', mesh?.actuation_authority === false ? 'NONE' : text(mesh?.actuation_authority, 'UNKNOWN'), mesh?.actuation_authority === false ? 'good' : 'neutral'),
    kvRow('Instances', Array.isArray(mesh?.instances) ? mesh.instances.length : 'UNKNOWN', Array.isArray(mesh?.instances) ? 'neutral' : 'neutral'),
  );
  fragment.append(meshSection.wrap);
  return fragment;
}
...
function renderDevos(next) {
  const fragment = document.createDocumentFragment();
  const sup = next?.supervisor;
  const cycle = sup?.devos_task_cycle;
  if (!cycle) {
  ...
  transport.list.append(
    ...
    kvRow('Scheduler source', sup?.devos_scheduler_source || 'UNKNOWN', sup?.devos_scheduler_source ? 'good' : 'neutral'),
    kvRow('Second loop', sup?.devos_second_polling_loop === false ? 'NO' : 'UNKNOWN', sup?.devos_second_polling_loop === false ? 'good' : 'neutral'),
  );
...
function renderRuntime(next) {
  ...
  runtime.list.append(
    kvRow('Development Plane', dev?.state || 'UNKNOWN', dev ? stateTone(dev.state) : 'neutral'),
    kvRow('Compute', compute ? (compute.available === true ? 'AVAILABLE' : 'OFFLINE') : 'UNKNOWN', compute ? (compute.available === true ? 'good' : 'bad') : 'neutral'),
    kvRow('Downloads', downloads?.state || downloads?.status || (downloads ? 'EXPOSED' : 'UNKNOWN'), downloads ? stateTone(downloads.state || downloads.status) : 'neutral'),
    kvRow('Worker observer', observer ? 'EXPOSED' : 'UNKNOWN', observer ? 'good' : 'neutral'),
    kvRow('Observer error', observer?.last_error || next?.supervisor?.worker_observation_last_error || 'NONE', observer?.last_error ? 'bad' : 'neutral'),
  );
  fragment.append(runtime.wrap);

  const updateSection = section('Self-update continuity', 'restart resumable');
  updateSection.list.append(
    kvRow('Updater state', update?.state || 'UNKNOWN', update ? stateTone(update.state) : 'neutral'),
    kvRow('Current version', update?.current_version || next?.version || 'UNKNOWN', update?.current_version || next?.version ? 'good' : 'neutral'),
    kvRow('Available version', update?.available_version || 'NONE', update?.available_version ? 'warn' : 'neutral'),
    kvRow('Install barrier', update?.install_effect_barrier_mode || 'UNKNOWN', update?.install_effect_barrier_mode ? 'good' : 'neutral'),
    kvRow('Automatic effect retry', update?.automatic_effect_retry === false ? 'DISABLED' : 'UNKNOWN', update?.automatic_effect_retry === false ? 'good' : 'neutral'),
    kvRow('Session continuity', continuity?.state || 'UNKNOWN', continuity ? stateTone(continuity.state) : 'neutral'),
  );
  fragment.append(updateSection.wrap);
  return fragment;
}
...
function renderCommands() {
  ...
  list.append(
    commandButton('New ChatGPT tab', 'Ctrl+N style local tab action', () => api.command('NEW_CHATGPT', {})),
