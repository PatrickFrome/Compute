...
async function upsertStateRow(row:any){
  await sql.unsafe(`insert into public.${STATE_TABLE}(client_id,workspace_id,last_seen_at,extension_version,operator_runtime,supervisor_mode,armed,operator_mode,ordering_policy,last_command_id,last_command_status,state,authority_effect) values($1,$2::uuid,clock_timestamp(),$3,$4,$5,$6::boolean,$7,$8,$9::uuid,$10,$11::jsonb,$12::boolean) on conflict(client_id) do update set workspace_id=excluded.workspace_id,last_seen_at=excluded.last_seen_at,extension_version=excluded.extension_version,operator_runtime=excluded.operator_runtime,supervisor_mode=excluded.supervisor_mode,armed=excluded.armed,operator_mode=excluded.operator_mode,ordering_policy=excluded.ordering_policy,last_command_id=excluded.last_command_id,last_command_status=excluded.last_command_status,state=excluded.state,authority_effect=excluded.authority_effect`,[row.client_id,row.workspace_id,row.extension_version,row.operator_runtime,row.supervisor_mode,row.armed,row.operator_mode,row.ordering_policy,row.last_command_id,row.last_command_status,row.state,row.authority_effect]);
  return row;
}
...
function boundedState(value:any){const s=value&&typeof value==='object'?value:{};const tabs=Array.isArray(s.tabs)?s.tabs.slice(0,64).map((t:any)=>({tab_id:String(t?.tab_id||'').slice(0,80),url:String(t?.url||'').slice(0,1200),title:String(t?.title||'').slice(0,240),kind:String(t?.kind||'').slice(0,40),selected:t?.selected===true})):[];return{schema:'metaengine.native-browser-supervisor.state.v1',client_kind:'METAENGINE_BROWSER_ELECTRON_NATIVE',shell_version:String(s.shell_version||'').slice(0,32),supervisor_mode:modeOf(s.supervisor_mode),armed:s.armed===true,operator_mode:String(s.operator_mode||'CONTROL').slice(0,32),tabs,active_tab:s.active_tab&&typeof s.active_tab==='object'?s.active_tab:null,development_plane:boundedObject(s.development_plane,32768),compute:boundedObject(s.compute,32768),fleet:boundedObject(s.fleet,65536),perception:boundedObject(s.perception,32768),supervisor_lifecycle:boundedObject(s.supervisor_lifecycle,32768),supervisor_mesh:boundedMesh(s.supervisor_mesh),self_update:boundedObject(s.self_update,32768),host_resilience:boundedObject(s.host_resilience,32768),realtime_process_plane:boundedObject(s.realtime_process_plane,262144),control_latency:boundedObject(s.control_latency,32768),realtime_observation_push:s.realtime_observation_push===true,last_error:String(s.last_error||'').slice(0,500)||null,started_at:s.started_at||null,heartbeat_at:new Date().toISOString()}}
