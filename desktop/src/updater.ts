 ...
 *
 * ⛔ DEPRECATED (R50, решение K6/K2 из docs/electron-rebuild-plan.md, фаза B):
 * авторитетная оболочка единой системы — METAENGINE Browser (apps/metaengine-browser,
 * ветка release/self-update-ambiguity-live-v2): self-update-runtime-v8 + Guardian +
 * CI autorelease уже доставляют смарт-мержи живым установкам (пруф: PR #948/#950/#951,
 * релизы v0.7.0-dev.*). Два updater'а в одной системе — гонка, поэтому ЭТОТ updater
 * выведен из эксплуатации: код сохранён как источник механизмов (semver-выбор артефакта,
 * staged-скачивание), НЕ подключать к новым сборкам. Доставка единой версии — только
 * через штатный self-update браузера.
 *
