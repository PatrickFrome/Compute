...
usage() {
  cat <<'EOF'
Usage: sudo ./install.sh --worker-id ID --gateway-url HTTPS_URL [--bearer-token-file PATH] [--heartbeat-interval SEC] [--enable]
...
for f in w1_safety_probe.py worker_agent.py metaengine-worker.service; do
...
printf 'Installed worker_id=%s\nunit_sha256=%s\ngateway_host=%s\npinned_gateway_ips=%s\npinned_dns_ips=%s\n' \
  "$WORKER_ID" "$unit_sha" "$gateway_host" "${gateway_ips[*]}" "${dns_ips[*]}"
