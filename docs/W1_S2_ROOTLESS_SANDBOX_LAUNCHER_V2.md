...
7. Additional host binds are read-only only; `/`, `/proc`, `/sys`, `/dev`, `/run`, and `/var/run` sources are rejected. The workspace may not be `/` or sensitive runtime/config roots, and workspace/sandbox-root overlap is rejected. Linux 5.12+ `mount_setattr(..., AT_RECURSIVE)` applies read-only/nosuid/nodev policy to nested mounts rather than hardening only the bind root. Recursive `NODEV` is waived only for the four exact launcher-owned bindings `/dev/null`, `/dev/zero`, `/dev/random`, and `/dev/urandom`.
...
10. The command vector is bounded and resolved only through a fixed launcher `PATH`. The worker applies `PR_SET_NO_NEW_PRIVS`, the substantive seccomp policy (including cross-process memory, `io_uring`, legacy and file-descriptor-based mount APIs, `fsconfig`, and `pivot_root`), capability bounding-set drops and a private umask. It receives a fixed credential-free environment, atomically detaches/closes inherited descriptors with `close_range(..., CLOSE_RANGE_UNSHARE)`, then uses `execve`.
...
## Effective host floor

The exact launcher requires Linux 5.12 or newer. `mount_setattr` (Linux 5.12)
is mandatory for recursive mount hardening; `close_range` (Linux 5.9) and the
pidfd APIs are also mandatory. Missing kernel or Python pidfd support is a
fail-closed exit 78, never a compatibility fallback. A name-only `clone3`
deny is intentionally absent because it could block normal process/thread
creation; any future rule must filter only dangerous namespace flags.

## Required rail review before execution

- Exact source/test/docs are fetched from one immutable Git commit.
- Recompute SHA-256 and match `25586bd8e0e97a78988f93d9c68c358b7eea6924015d06721afc135412d386df`.
