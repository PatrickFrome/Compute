...
| Independent exact runtimes for 30 minutes | BLOCKED | No independently hosted exact GPT/GLM runtime endpoints or credentials were available to this implementation session |
...
Do not mark PR #49 ready and do not call A2 production-ready until CI is green, the HTTP ingress Ed25519 end-to-end canary passes, and the two independently hosted exact runtimes complete the 30-minute no-relay canary. The 30-minute canary must include disconnect/reconnect, P0/P1 interruption, a real tool result observed by both peers, conflict escalation into SAME_POINT_DUEL_V4, and a mutation attempt that succeeds only after fresh executor authority revalidation.
