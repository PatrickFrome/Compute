...
4. **Formal safety model.** `formal/A2CausalBus.tla` captures cursor ordering, no mandatory-event loss, fresh acceptance, mode validity and permanent non-authority. TLC checked the bounded model through `MaxSeq=5`: 9,055 states generated, 1,902 distinct states, depth 14, no invariant violation. Apalache can add symbolic bounded checks locally: <https://apalache-mc.org/docs/apalache/running.html>.
