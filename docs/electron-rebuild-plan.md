...
### Фаза C — единый monorepo (R51-R52, P1) — C6 ЗАКРЫТ (R51) · C7 ЗАКРЫТ (R52, PR #953)
...
7. ✅ **C7 ЗАКРЫТ (R52, PR #953)**: `pack-me2-ui.mjs` (standalone → `me2-ui-dist`, контракт ui-host, манифест `me2.ui-bundle-manifest.v1`) — локально проверен: build → pack 69.4 MiB → smoke HTTP 200 HTML; `extraResources me2-ui-dist → me2-ui` в electron-builder (панели v5 едут в установщике); шаг build+pack (fail-open, честный README-стаб без package.json — ui-host не усыновляет мёртвый каталог) в `browser-final-runtime-activation-v1.yml`; job `ui-build` в me2-unified-gate (frozen → build → pack → verify → smoke → artifact). Паттерн «один SHA → один релиз» (VS Code) — research/2026/r52-analogues.md.

### Фаза D — Supabase-контур (R53, P1, H6) — D-prep ЗАКРЫТ (R52) · D-исполнение: инфраструктура ГОТОВА (R53), DDL — ожидает креденшал
8. ✅ **DEPRECATED-реестр (R52)**: живой инвентарь OpenAPI (243 RPC) классифицирован: **ACTIVE 24** (aop1_* vercel gateway, a2_supervisor_mesh_*, evidence), **CONTROL_PLANE 37** (worker_enrollment_*, actuation_lease, supervisor_command/enqueue/lease, device_nonce, pairing, gate_policy — держат живые таблицы), **FREEZE 182** (duel/macroblock/fabric/fail_run/legacy a2_*). Реестр — ДАННЫМИ, не молчанием: `sql/0002-rpc-registry.sql` (таблица me2_rpc_registry_h205f22, idempotent upsert) + research/2026/r52-rpc-registry.json + docs/supabase-rpc-registry.md. FREEZE = пометить, НЕ удалять (решение — оператор после 30 дней наблюдения зеркала).
9. ✅ **H6 SQL-контур (R52)**: daemon v0.44.0 `src/sqlmirror.ts` — зеркало hash-chain событий в SQL-таблицу (PostgREST, idempotent по PK seq, ignore-duplicates); operator-gated `ME2_SQL_MIRROR=1` + vault-ключ R47; честные состояния OFF/WARMUP/LIVE/DEGRADED (перепроба ≤ PROBE_MS — PGRST205-протокол без штормов); `GET /sqlmirror` read-only вне шины (47/47 не тронут). WARMUP-проба живая подтверждена (table_missing_404, шторма нет). Миграция `sql/0001-me2-event-mirror.sql` (таблица+RLS+индексы) — оператору; после неё контур сам WARMUP→LIVE.
10. ✅ **D-исполнение (R53)**: инструкция оператора «применить самому» разобрана честной инвентаризацией: `$DATABASE_URL` песочницы = локальная SQLite (file:); прямой 5432 db.<ref>.supabase.co заблокирован сетью, но **Supavisor pooler :5432 достижим**; psql собран без root (postgresql-client-17, dpkg -x + системный libpq5); DB-пароля в sandbox нет (vault = 5 ключей; aop1-секретарь PGRST202 ×2 — без штормов). Готово: `sql/0003-mirror-read-policy.sql` (RLS read-политики authenticated, anon fail-closed); `scripts/apply-sql-migrations.sh` (идемпотентное применение 0001→0002→0003 с маской секретов и верификацией) + **самоприменение из start.sh** — как только оператор добавит `SUPABASE_DB_URL` в `/home/z/.a2/supabase-cloud.env`, система применит миграции сама (человек-шаг исчезает); гейт включён (`ME2_SQL_MIRROR=1` в start.sh) — daemon в WARMUP и перейдёт LIVE сам. Чтение из UI с RLS-гейтом: `src/supabase-jwt.ts` двухрежимный read-канал (publishable sb_publishable_ — канонический / mint HS256 120с — legacy), REST `/sqlmirror/ui-token` + `/verify`, панель «Зеркало SQL (RLS)» в GET /ui (anon-проба + предупреждение о протечке). **Живая проба открыла: ключ vault'а — sb_secret_… (не JWT), облако отвергает HS256-mint (401) → ожидается sb_publishable от оператора; канал честно деградирует словами.** eval v18→v19: +contract.supabase_jwt, +mission.sqlmirror_ui — PASS 55/55.

### Фаза E — гигиена (R53-R54+, P2) — E10 НАЧАТА (R53): 292/1118
11. ✅ **Архивация раунд-1 (R53)**: `research/2026/r53_archive_branches.py` (stdlib-only: heads одним ответом — /git/refs/heads игнорирует пагинацию, GraphQL-даты пачками ×100, фенс открытых PR 584, манифест JSON). Исполнено: **292 ветки** (work/fix/repair/ops старше 14 дней вне открытых PR) → тег `archive-2026-09-*` + удаление, 0 ошибок за 105с; голов 1168→876; данные восстановимы с тегов. Остаток: 581 (открытые PR) + 245 (<14д) — следующие окна по мере старения/закрытия PR.
12. ✅ **Матрица версий (K8, R52)**: живая сторона — `GET /state → capabilities.compat` (contract/daemon/ui_fallback/browser_expect/notes); человеческая проекция + история контракта — `docs/version-matrix.md`.
