...
## §10 SEC-SEALED-3 (2026-09-27) — 4 канала выживания + авто-аудит
- Канал 1 (главный, распоряжение оператора): `scripts/phoenix/phoenix-secrets-restore.sealed.sh` — живые литералы ПРЯМО в скрипте (chmod 600, git-ignored: scripts/phoenix/*.sealed.*). Канал 2: PolarFS /tmp/my-project/phoenix-sealed/ + phoenix-sealed-local.sh. Канал 3: ossfs /home/sync/me2-context-backups/phoenix-sealed/. Канал 4: cron 416759 (эталон литералов в payload, вне песочницы).
- Восстановление одной командой: `bash scripts/phoenix/phoenix-secrets-restore.sealed.sh full` = gitguard + restore(.github.env/ENVF/.env) + validate(HTTP-коды) + mirrors(3 канала) + АВТО-АУДИТ. Оркестратор v1.1 тоже запускает аудит автоматически.
- full-audit.sh v2.0 (cron 416761, 6ч): score 73% (AUD-2, DONE=15 PARTIAL=1 BLOCKED=5). До 100%: перевыпуск Supabase service JWT + (опционально) R2 secret access key. Отчёты: audit/audit-*.md, state: audit/.audit-state.
