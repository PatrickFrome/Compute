// R60: VLM-верификация операторских скриншотов (r60-workbench, r60-workbench-mobile).
// Канон r58-vlm-qa.ts: sdk ТОЛЬКО в backend-скрипте; честные вердикты, не фантазии.
// Запуск: cd mini-services/me2-daemon && bun ../../research/2026/r60-vlm-qa.ts
...
const SHOTS: Array<{ file: string; expect: string[] }> = [
  {
    file: "download/r60-workbench.png",
    expect: ["заголовок ME2 Mission Control и чип daemon 0.48.0", "секция «Расширения (exthost · prlimit)» со строкой mirror-digest и кнопкой «Запустить»", "кнопка «лэйаут ↺» в шапке", "строки аудита политик и сверки реестра в секции Зеркало SQL"],
  },
  {
    file: "download/r60-workbench-mobile.png",
...
async function main() {
  ...
  writeFileSync(`${ROOT}/research/2026/r60-vlm-qa1.json`, JSON.stringify(out, null, 2));
  console.log(JSON.stringify(out.shots.map((s: any) => ({ file: s.file, ok: s.ok, error: s.error ?? undefined })), null, 2));
  console.log("written: research/2026/r60-vlm-qa1.json");
}
