...
// ── v5 (R45): панели консоли — главная БРАУЗЕР (чаты-агенты + живой сайт), остальные по вкладкам ──
type PanelKey = "browser" | "fleet" | "mission" | "telemetry" | "log";
const PANELS: { key: PanelKey; label: string; icon: LucideIcon; hint: string }[] = [
  { key: "browser", label: "БРАУЗЕР", icon: AppWindow, hint: "главная: чаты-агенты + живой сайт на экране (как в старом браузере)" },
  { key: "fleet", label: "ФЛОТ", icon: Bot, hint: "сетка чатов с целями (G6), агенты, пул, workers" },
  { key: "mission", label: "МИССИЯ", icon: Target, hint: "ветки-задачи, цели, механики, очередь, шина" },
  { key: "telemetry", label: "ТЕЛЕМЕТРИЯ", icon: Activity, hint: "сенсоры, eval, автономия V4, governor, гигиена" },
  { key: "log", label: "ЖУРНАЛ", icon: Terminal, hint: "event log — река всех событий" },
];
...
export default function MissionControl() {
  ...
  // v5 (R45): активная панель + сайдбар чатов + nonce реконнекта стрима.
  // Панель монтируется условно (перф), выбор персистится в localStorage + hash.
  const [panel, setPanel] = useState<PanelKey>("browser");
  const [chatSidebarOpen, setChatSidebarOpen] = useState(true);
  const [castNonce, setCastNonce] = useState(0);
  const panelRef = useRef<PanelKey>("browser");
  const switchPanel = useCallback((p: PanelKey) => {
    panelRef.current = p;
    setPanel(p);
  ...
  // v5: по умолчанию ВКЛЮЧЁН — главная панель БРАУЗЕР обязана показывать сайт на экране
  ...
  // v5: восстановление панели из hash/localStorage (после гидрации — без mismatch)
  useEffect(() => {
    const t = window.setTimeout(() => {
      const h = window.location.hash.replace("#", "");
      let stored: string | null = null;
      try { stored = localStorage.getItem("me2.panel.v1"); } catch { /* приватный режим */ }
      const raw = PANELS.some((p) => p.key === h) ? h : stored;
      if (raw && PANELS.some((p) => p.key === raw) && raw !== panelRef.current) switchPanel(raw as PanelKey);
    }, 0);
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => { void loadBrowserTabs(); }, 0);
    const iv = window.setInterval(() => { void loadBrowserTabs(); }, 45_000);
    return () => { clearTimeout(t); clearInterval(iv); };
  }, [panel, loadBrowserTabs]);

  // v5: клик по карточке флота (G6) открывает чат на главной панели БРАУЗЕР
  // (если чат-панель не смонтирована — переключаемся и переотправляем выбор после монтирования)
  useEffect(() => {
    const onSel = (e: Event) => {
      if (panelRef.current === "browser") return;
      switchPanel("browser");
      const id = (e as CustomEvent<{ id?: string }>).detail?.id;
      if (id) window.setTimeout(() => window.dispatchEvent(new CustomEvent("me2:select-chat", { detail: { id } })), 400);
    };
    window.addEventListener("me2:select-chat", onSel as EventListener);
    return () => window.removeEventListener("me2:select-chat", onSel as EventListener);
  }, [switchPanel]);
  ...
  return (
    <div className="mc-dark flex min-h-screen flex-col bg-zinc-950 text-zinc-100 lg:h-screen">
      {/* ── HEADER v5: бренд + ПАНЕЛИ-ВКЛАДКИ (как в браузере) + KPI ── */}
      ...
      {/* ── MAIN v5: панельный шелл — активная панель монтируется условно (перф: невидимые панели не рендерятся) ── */}
          ...
          <section className="mc-scroll min-h-0 flex-1 overflow-y-auto p-3" aria-label="Флот" data-testid="panel-fleet">
            <div className="flex flex-col gap-4">
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="fleet-pool">
                        <CardContent className="p-3">
                      {/* R34 E3: EXECUTOR·POOL — параллельные живые GLM-исполнители с честными lease (ME33, GET/POST /pool) */}
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="Пул исполнителей">
          ...
          <section className="mc-scroll min-h-0 flex-1 overflow-y-auto p-3" aria-label="Миссия" data-testid="panel-mission">
            <div className="grid items-start gap-4 lg:grid-cols-2 2xl:grid-cols-3">
              <div className="flex min-w-0 flex-col gap-4">
                      <Card className={`flex min-h-0 flex-col gap-0 overflow-hidden border-zinc-800 bg-zinc-900/40 py-0 card-lift lg:max-h-[52vh]`}>
        ...
        {panel === "telemetry" && (
          <section className="mc-scroll min-h-0 flex-1 overflow-y-auto p-3" aria-label="Телеметрия" data-testid="panel-telemetry">
            <div className="grid items-start gap-4 lg:grid-cols-2">
              <div className="flex min-w-0 flex-col gap-4">
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="tl-sense">
                        <CardContent className="p-3">
                      {/* R20: SENSE — семантическая перцепция вкладки (порт легаси CAPTURE→act→verify, ME17) */}
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="Семантическая перцепция страницы">
                        <div className="flex items-center gap-2">
                      ...
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="tl-obsv">
                        <CardContent className="p-3">
                      {/* R21: OBSV — network/console/exceptions сенсоры вкладки (S2, ME18, Chrome DevTools MCP parity) */}
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="Сенсоры сети и консоли вкладки">
                        <div className="flex items-center gap-2">
                      ...
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="tl-bench">
                        <CardContent className="p-3">
                      {/* R25: BENCH — перф-бейслайны daemon (ME20, GET /bench) + живость MCP-сервера (ME21) */}
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="Перф-бейслайны daemon и MCP-сервер">
                        <div className="flex items-center gap-2">
                      ...
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="tl-eval">
                        <CardContent className="p-3">
                      {/* R26 B1: EVAL — регресс-датасет + eval-харнесс (ME22, GET /eval | POST /eval/run) */}
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="Регресс-датасет и eval-харнесс daemon">
                        <div className="flex items-center gap-2">
              ...
              <div className="flex min-w-0 flex-col gap-4">
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="tl-hygiene">
                        <CardContent className="p-3">
                      {/* R31 D4: DB·HYGIENE — гигиена SQLite daemon (ME30, GET/POST /db/hygiene) */}
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="Гигиена базы данных daemon">
                        <div className="flex items-center gap-2">
                      ...
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="tl-evchain">
                        <CardContent className="p-3">
                      {/* R33 E2: EVIDENCE·CHAIN — верификация hash-chain audit-trail (ME32, GET /evidence/verify; IETF agent-audit-trail) */}
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="Верификация цепочки доказательств">
                        <div className="flex items-center gap-2">
                      ...
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="tl-autonomy">
                        <CardContent className="p-3">
                      ...
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="Плоскость автономии v4">
                        <div className="flex items-center gap-2">
                      ...
                      <Card className="shrink-0 border-zinc-800 bg-zinc-900/40 card-lift" data-testid="tl-governor-demand">
                        <CardContent className="p-3">
                      {/* R43 G11+G10: GOVERNOR (полосы LLM-спроса + circuit breaker) и АВТОПИЛОТ СПРОСА (daemon-demand → create_chat) */}
                      <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40" aria-label="LLM-Governor и автопилот спроса">
