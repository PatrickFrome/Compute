...
export function parseUserUrl(input) {
  ...
  // GLM agent platform (2026-09-19): the browser home is the agent platform
  // root — the fleet's first provisioning surface.
  if (!raw) return new URL(AGENT_PLATFORM_HOME_URL);
...
export function isGlmChatUrl(input) {
  const x = classifyRemoteUrl(input);
  return x.allowed && x.kind === 'GLM_CHAT';
}

export function isChatSurfaceUrl(input) {
  const x = classifyRemoteUrl(input);
  return x.allowed && (x.kind === 'CHATGPT' || x.kind === 'GLM_CHAT');
}
