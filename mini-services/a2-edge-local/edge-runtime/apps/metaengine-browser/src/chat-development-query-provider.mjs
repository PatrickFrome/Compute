...
function normalizeOrientation(value) {
  ...
  if (!plain(value) || value.authority_effect === true) throw new Error('chat_dev_provider_orientation_invalid');
  const revision = String(value.revision || '').trim();
  if (!revision || revision.length > 96) throw new Error('chat_dev_provider_orientation_revision_invalid');
  ...
  return Object.freeze({
    ...
    ci_failed: Number.isSafeInteger(Number(value.ci_failed)) ? Number(value.ci_failed) : 0,
    ci_pending: Number.isSafeInteger(Number(value.ci_pending)) ? Number(value.ci_pending) : 0,
    focus_kind: clip(value.focus_kind, 32),
    focus_id: clip(value.focus_id, 160),
    focus_title: clip(value.focus_title, 220),
    authority_effect: false,
  });
}
...
export class ChatDevelopmentQueryProvider {
  ...
  async query({ query, kinds = null, limit = 8, if_none_match = null, max_bytes = CHAT_DEVELOPMENT_MAX_RESULT_BYTES, orientation = null } = {}) {
    ...
    const requestKey = sha256(JSON.stringify({
      ...
      orientation_revision: normalizedOrientation?.revision ?? null,
    }));
    ...
    return fit({
      ...
      one_call_orientation: normalizedOrientation != null,
