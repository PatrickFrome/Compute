-- Deny-only scheduler backpressure. Browser/page text can suppress a lease but can
-- never authorize one. This prevents READY work from becoming 15-minute LEASED
-- ambiguity debt while the authenticated native Browser reports a fresh account-
-- level ChatGPT rate-limit surface.
...
declare
  ...
  v_browser_state text := null;
begin
  select s.state #>> '{}'
    into v_browser_state
  ...
  if lower(coalesce(v_browser_state, '')) ~
     '(too many requests|слишком много запросов|temporarily limited|временно ограничен)' then
  ...
  insert into destruktion_meta.devos_fleet_claim_h205f22(
    task_id, workspace_id, point_id, base_sha, role, claim_class,
    agent_id, tab_id, target_id, agent_generation_epoch, lease_generation, expires_at
  ) values (
    v_task.task_id, v_task.workspace_id, v_task.point_id, v_task.base_sha,
    v_task.role, v_task.claim_class, lower(p_agent), p_tab, lower(p_target),
    p_epoch, v_task.lease_generation, v_task.lease_expires_at
  )
