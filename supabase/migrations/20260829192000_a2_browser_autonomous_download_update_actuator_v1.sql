...
begin
  ...
  select coalesce(sum(case
      when action in ('POLL','CAPTURE','DOWNLOAD_STATUS','SELF_UPDATE_STATUS','DISARM') then 0
  ...
  where workspace_id=p_workspace_id and status='PENDING' and expires_at>v_now
    ...
    and (v_mode='CONTROL' or action in ('SET_SUPERVISOR_MODE','ARM','DISARM','POLL','CAPTURE','CAPTURE_VIEW','DOWNLOAD_STATUS','SELF_UPDATE_STATUS'))
  ...
  v_requested_cost := case
    when v_row.action in ('POLL','CAPTURE','DOWNLOAD_STATUS','SELF_UPDATE_STATUS','DISARM') then 0
