-- Branch-local C0 hardening: verify durable composed recovery/restart precondition proof without granting authority.
create extension if not exists pgcrypto;

create or replace function public.h205f22_verify_compute_unified_recovery_restart_precondition_proof_v1(
  p_workspace_id uuid,
  p_recovery_attempt_id text,
  p_restart_intent_id text,
  p_recovery_restart_precondition_fingerprint_sha256 text
) returns jsonb language plpgsql stable security definer set search_path='' as $$
declare
  v public.compute_unified_recovery_restart_precondition_proof_h205f22%rowtype;
  v_fp text;
  v_e jsonb;
begin
  if p_workspace_id is null or nullif(p_recovery_attempt_id,'') is null or nullif(p_restart_intent_id,'') is null
     or p_recovery_restart_precondition_fingerprint_sha256 !~ '^[0-9a-f]{64}$' then
    raise exception 'recovery/restart readback identity/fingerprint invalid';
  end if;
  ...
  select * into v
   ...
   where workspace_id=p_workspace_id
     ...
     and recovery_restart_precondition_fingerprint_sha256=p_recovery_restart_precondition_fingerprint_sha256;

  if v.recovery_restart_precondition_proof_id is null then
    raise exception 'recovery/restart durable proof not found';
  end if;

  v_e := v.verified_evidence;
  v_fp := encode(public.digest(convert_to(v_e::text,'UTF8'),'sha256'),'hex');

  if v_fp is distinct from v.recovery_restart_precondition_fingerprint_sha256
     or v_fp is distinct from p_recovery_restart_precondition_fingerprint_sha256
     or v_e->>'schema' is distinct from 'metaengine.compute-unified.recovery-restart-precondition.v1'
     or not coalesce((v_e->>'preconditions_verified')::boolean,false)
     or v_e->>'reason' is distinct from 'RECOVERY_AND_TYPED_RESTART_PRECONDITIONS_VERIFIED'
     ...
     or v_e->>'recovery_admission_fingerprint_sha256' is distinct from v.recovery_admission_fingerprint_sha256
    ...
    raise exception 'recovery/restart durable proof integrity/provenance verification failed';
  end if;

  return jsonb_build_object(
    'schema','metaengine.compute-unified.recovery-restart-precondition-readback.v1',
    'verified',true,
    'reason','RECOVERY_RESTART_PRECONDITION_DURABLE_PROOF_VERIFIED',
...
revoke all on function public.h205f22_verify_compute_unified_recovery_restart_precondition_proof_v1(uuid,text,text,text) from public,anon,authenticated;
grant execute on function public.h205f22_verify_compute_unified_recovery_restart_precondition_proof_v1(uuid,text,text,text) to service_role;
