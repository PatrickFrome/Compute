-- METAENGINE Browser agent observation & action plane v1 (2026-09-19).
-- Operator directive: browser, supervisors and agents must see absolutely all
-- browser processes in real time, with maximum action variability.
--
-- New READ_ONLY observation actions (bounded, metadata-only, zero authority):
--   TAB_TELEMETRY    — per-tab console/network/health ring buffer tail
--   SYSTEM_TELEMETRY — single-trace digest of every browser process surface
--   READ_TRANSCRIPT  — paged bounded conversation text read (60k ceiling)
-- New TAB_MUTATION input action (geometry-free, effect-bound):
--   PRESS_KEY        — whitelisted non-printing key dispatch to a tab

-- 1) Extend the command action domain.
...
alter table public.compute_fabric_a2_browser_supervisor_command_h205f22
  ...
  check (action = ANY (ARRAY[
    ...
    'SELF_UPDATE_STATUS','SELF_UPDATE_CHECK','SELF_UPDATE_APPLY','DEVELOPER_EMERGENCY_UPDATE',
    'GATE_STATUS','GATE_DISABLE','GATE_DISABLE_ALL','GATE_ENABLE','GATE_ENABLE_ALL'
  ]));

-- 2) Regenerate the lane/effect classification with the new actions.
alter table public.compute_fabric_a2_browser_supervisor_command_h205f22
...
-- 3) Extend the issuer RPC whitelist (v3: identical contract, four new actions).
...
begin
  ...
  if v_action not in (
    ...
    'TAB_TELEMETRY','SYSTEM_TELEMETRY','READ_TRANSCRIPT',
    'STOP_GENERATION','SCROLL','SEMANTIC_FOCUS','SEMANTIC_TYPE','TYPED_CLICK','PRESS_KEY',
    'NEW_TAB','SELECT_TAB','CLOSE_TAB','NAVIGATE','BACK','FORWARD','RELOAD',
