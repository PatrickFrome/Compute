-- A2 Browser Supervisor v3: remote bootstrap authority.
-- The remote supervisor may bootstrap CONTROL/ARM/DISARM without leasing ordinary
-- browser actions while local supervisor_mode is OFF or MONITOR.
...
create or replace function public.h205f22_a2_browser_supervisor_lease_bootstrap_v3(
  p_workspace_id uuid,
  p_client_id text,
...
begin
  ...
  where workspace_id=p_workspace_id
    and status='PENDING'
    and expires_at > clock_timestamp()
    and action in ('SET_SUPERVISOR_MODE','ARM','DISARM')
    and (target_client_id is null or target_client_id=v_client)
  order by
    case
      when action='DISARM' then 0
      when action='SET_SUPERVISOR_MODE' and upper(coalesce(payload->>'mode',''))='OFF' then 1
      else 2
    end,
    issued_at asc
...
revoke all on function public.h205f22_a2_browser_supervisor_lease_bootstrap_v3(uuid,text,integer) from public, anon, authenticated;
...
grant execute on function public.h205f22_a2_browser_supervisor_lease_bootstrap_v3(uuid,text,integer) to service_role;
