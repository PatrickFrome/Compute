-- METAENGINE DevOS transport-promotion explicit JSON-null admission repair v1.
-- Branch-local candidate only. Production DDL requires the existing trusted release gate.
--
-- Root cause: jsonb extraction of an explicit JSON null returns the JSONB value 'null',
-- not SQL NULL. The v1 predicate "v_agent->'transport_proof' is not null" therefore
-- rejected every correctly serialized BOUND_UNVERIFIED agent whose transport_proof
-- field was explicitly null. Preserve fail-closed shape by requiring the key to exist
-- AND requiring jsonb_typeof(...) = 'null'; missing or non-null proof remains fenced.
--
-- Original purpose: break the restart deadlock where Browser agents are deliberately restored as
...
begin
  ...
  if v_agent->>'ownership'<>'FLEET_OWNED'
     ...
     or not (v_agent ? 'transport_proof')
     or jsonb_typeof(v_agent->'transport_proof') <> 'null'
