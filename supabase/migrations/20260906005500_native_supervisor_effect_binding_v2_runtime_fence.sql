-- Native Supervisor effect binding v2: add runtime generation evidence while keeping
-- the existing DB lease as the only authority source. v1 remains accepted for
-- backwards compatibility; v2 is stricter local execution evidence, not a new lease.
...
declare
  ...
  v_schema text := coalesce(p_binding->>'schema','');
...
begin
  ...
  if v_schema = 'metaengine.native-supervisor.effect-binding.v2' then
    ...
    if coalesce(p_binding->>'attachment_generation','') !~ '^[1-9][0-9]{0,15}$' then raise exception 'native_effect_binding_attachment_generation_invalid'; end if;
    if coalesce(p_binding->>'document_generation','') !~ '^[1-9][0-9]{0,15}$' then raise exception 'native_effect_binding_document_generation_invalid'; end if;
    if coalesce(p_binding->>'binding_generation','') !~ '^[1-9][0-9]{0,15}$' then raise exception 'native_effect_binding_generation_invalid'; end if;
    if coalesce(p_binding->>'document_url_sha256','') !~ '^[0-9a-f]{64}$' then raise exception 'native_effect_binding_document_url_hash_invalid'; end if;
    if p_binding->>'runtime_observation_schema' is distinct from 'metaengine.native-supervisor.effect-runtime-observation.v1' then
      raise exception 'native_effect_binding_runtime_observation_schema_invalid';
    end if;
    if p_binding->>'target_id' is distinct from ('webcontents:' || (p_binding->>'web_contents_id')) then
