...
'Exact non-authoritative native supervisor runtime capability attestation. Service-role only; grants no scheduler, browser, retry, or effect authority.';
...
comment on function public.devos_recovery_debt_snapshot_v1(uuid) is
'Advisory recovery-debt partition from durable transport proof. Returns no task content, replays no physical effect, and grants no retry or scheduling authority.';
