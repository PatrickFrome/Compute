-- A2 Browser Supervisor v2: idempotent issuance + terminal stale-lease reap.
-- Additive only: v1 RPCs remain available for rollback compatibility.
...
begin
  if not exists (
    ...
    where conrelid = 'public.compute_fabric_a2_browser_supervisor_command_h205f22'::regclass
      and conname = 'a2_browser_supervisor_command_idempotency_key_ck'
  ) then
    alter table public.compute_fabric_a2_browser_supervisor_command_h205f22
      add constraint a2_browser_supervisor_command_idempotency_key_ck
      check (
...
create unique index if not exists compute_fabric_a2_browser_supervisor_command_idempotency_uidx
  on public.compute_fabric_a2_browser_supervisor_command_h205f22(workspace_id, idempotency_key)
  where idempotency_key is not null;

create or replace function public.h205f22_a2_browser_supervisor_enqueue_v2(
  p_workspace_id uuid,
  p_action text,
...
begin
  if v_action not in ('ARM','DISARM','SET_MODE','POLL','CAPTURE','STOP_GENERATION','SCROLL','SEMANTIC_FOCUS','SEMANTIC_TYPE','RESOLVE_PROMPT') then
  ...
  if not found then
    raise exception 'supervisor_idempotency_readback_missing';
  end if;
...
begin
  ...
  -- A stale lease is terminal. It is never requeued because execution may have
  -- crossed an irreversible boundary before the worker/ack was lost.
  update public.compute_fabric_a2_browser_supervisor_command_h205f22
...
revoke all on function public.h205f22_a2_browser_supervisor_enqueue_v2(uuid,text,text,jsonb,text,integer,text,text) from public, anon, authenticated;
revoke all on function public.h205f22_a2_browser_supervisor_lease_v2(uuid,text,integer) from public, anon, authenticated;
grant execute on function public.h205f22_a2_browser_supervisor_enqueue_v2(uuid,text,text,jsonb,text,integer,text,text) to service_role;
