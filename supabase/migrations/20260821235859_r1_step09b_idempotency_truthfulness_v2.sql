...
declare
  ...
  v_audit jsonb;
  v_evidence jsonb;
  v_base_evidence jsonb;
  v_domain_metadata jsonb;
  v_account_scope text;
  v_provider_result_sha text;
  v_obs_id bigint;
  ...
  v_object_inserted int := 0;
  v_object_reused int := 0;
  ...
  v_rowcount bigint;
  v_database_write_performed boolean;
begin
  if current_setting('transaction_isolation') <> 'read committed' then
    raise exception 'STEP09B requires read committed isolation' using errcode='25001';
  end if;
  ...
  for v_domain in select value from jsonb_array_elements(p_projection->'domain_insert_or_exact_match') loop
    if jsonb_typeof(v_domain->'metadata') <> 'object' then raise exception 'STEP09B domain metadata invalid' using errcode='22023'; end if;
    v_account_scope := v_domain#>>'{metadata,account_scope_sha256}';
    v_provider_result_sha := v_domain#>>'{metadata,provider_result_sha256}';
    if v_account_scope is null or v_account_scope !~ '^[0-9a-f]{64}$' then raise exception 'STEP09B domain account scope sha256 invalid' using errcode='22023'; end if;
    if v_provider_result_sha is null or v_provider_result_sha !~ '^[0-9a-f]{64}$' then raise exception 'STEP09B domain provider result sha256 invalid' using errcode='22023'; end if;
    if coalesce((v_domain#>>'{metadata,step08_projection_only}')::boolean,false) is not true then raise exception 'STEP09B domain projection marker missing' using errcode='23514'; end if;
    v_domain_metadata := jsonb_build_object('account_scope_sha256',v_account_scope,'registration_contract','STEP09B_STABLE_DOMAIN_IDENTITY_V1');
    ...
    values(v_domain->>'domain_key',v_domain->>'provider_kind',v_domain->>'operator_class',v_domain->>'failure_domain',v_domain->>'independence_basis',coalesce((v_domain->>'physical_region_independence_claimed')::boolean,false),v_domain_metadata)
    on conflict (domain_key) do nothing;
    get diagnostics v_rowcount = row_count;
    if v_rowcount=1 then v_domains_inserted:=v_domains_inserted+1; else v_domains_reused:=v_domains_reused+1; end if;
    ...
    if v_existing_domain.provider_kind is distinct from v_domain->>'provider_kind' or v_existing_domain.operator_class is distinct from v_domain->>'operator_class' or v_existing_domain.failure_domain is distinct from v_domain->>'failure_domain' or v_existing_domain.independence_basis is distinct from v_domain->>'independence_basis' or v_existing_domain.physical_region_independence_claimed is distinct from coalesce((v_domain->>'physical_region_independence_claimed')::boolean,false) or v_existing_domain.metadata is distinct from v_domain_metadata then
      raise exception 'STEP09B existing domain % does not exactly match stable domain identity',v_domain->>'domain_key' using errcode='23514';
    end if;
  end loop;
  ...
  get diagnostics v_rowcount = row_count;
  if v_rowcount=1 then v_object_inserted:=1; else v_object_reused:=1; end if;
  ...
  for v_obs in select value from jsonb_array_elements(p_projection->'observation_inserts') loop
    ...
    v_base_evidence := coalesce(v_obs->'evidence','{}'::jsonb);
    v_evidence := v_base_evidence || jsonb_build_object('step09b',jsonb_build_object('schema','metaengine.compute.r1-step09b-observation-binding.h205f22.v1','projection_sha256',v_projection_sha,'authority_gate_receipt_sha256',v_gate_sha,'package_sha256',p_authority_gate->>'package_sha256','source_head_sha',p_authority_gate->>'source_head_sha','trusted_root_context_sha256',p_authority_gate#>>'{trusted_root,context_sha256}'));

    select observation_id into v_obs_id
    from destruktion_meta.compute_continuity_observation_h205f22
    where object_id=v_object_id
      and domain_key=v_obs->>'domain_key'
      and readback_at=v_readback_at
      and status='VERIFIED'
      and observed_sha256=lower(v_obs->>'observed_sha256')
      and observed_bytes is not distinct from nullif(v_obs->>'observed_bytes','')::bigint
      and persisted_at is not distinct from v_persisted_at
      and (evidence-'step09b') is not distinct from v_base_evidence
      and evidence#>>'{step09b,projection_sha256}' is not distinct from v_projection_sha
      and evidence#>>'{step09b,package_sha256}' is not distinct from p_authority_gate->>'package_sha256'
      and evidence#>>'{step09b,source_head_sha}' is not distinct from p_authority_gate->>'source_head_sha'
  ...
  v_database_write_performed := (v_domains_inserted + v_object_inserted + v_observations_inserted) > 0;
  return jsonb_build_object('schema','metaengine.compute.r1-step09b-db-ingestion-result.h205f22.v1','classification','DATABASE_DERIVED_TWO_DOMAIN_READBACK_QUORUM','projection_sha256',v_projection_sha,'authority_gate_receipt_sha256',v_gate_sha,'object_id',v_object_id,'domains_inserted',v_domains_inserted,'domains_reused',v_domains_reused,'object_inserted',v_object_inserted,'object_reused',v_object_reused,'observations_inserted',v_observations_inserted,'observations_reused',v_observations_reused,'effective_at',v_effective_at,'continuity_readiness',v_readiness,'continuity_audit',v_audit,'database_transaction_validated',true,'database_write_performed',v_database_write_performed,'continuity_readiness_r2_proven',true,'canonical_roadmap_r2_promoted',false,'r3_proven',false,'persisted_seal_created',false,'required_next','SUPERVISOR_AUDIT_R1_AND_DECIDE_R1_SEAL_THEN_CANONICAL_R2_PROMOTION_SEPARATELY');
end;
$$;
...
comment on function destruktion_meta.compute_ingest_r2_projection_h205f22(jsonb,jsonb) is 'H205F22 STEP09B v2 postgres-only append-only ingestion. Requires READ COMMITTED; normalizes durable domain metadata to stable account/domain identity; permits idempotent reuse of an exact readback under a newly issued STEP09A gate without mutating evidence; reports whether any row was actually inserted; derives continuity R2 from live DB tables; never creates a persisted seal or canonical roadmap promotion.';
