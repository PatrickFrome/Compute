-- METAENGINE supervisor mesh bulk heartbeat sync.
-- Zero-authority telemetry: one signed Browser heartbeat refreshes all locally discovered supervisor peers.
  ...
  loop
    ...
    insert into public.compute_fabric_a2_supervisor_mesh_instance_h205f22(
      ...
      jsonb_build_object(
        ...
        'client_id_sha256',encode(digest(v_client,'sha256'),'hex')
      ),
...
is 'Zero-authority bulk sync for a signed native Browser supervisor mesh heartbeat. Accepts only bounded hash/id/status/tab metadata and cannot issue Browser commands.';
