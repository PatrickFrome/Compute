...
begin
  ...
  -- The caller identity comes from the signed native-supervisor route. A different enrolled
  -- device cannot recover another current CONTROL supervisor's ambiguity debt.
  v_control:=public.devos_control_supervisor_snapshot_v1(p_workspace,v_client,45);
  if v_control->>'state'<>'FRESH_CONTROL' then raise exception 'devos_ambiguity_supervisor_client_fenced' using errcode='55000'; end if;
  ...
  select * into v_claim from destruktion_meta.devos_fleet_claim_h205f22 c
   where c.workspace_id=p_workspace and c.task_id=p_task and c.agent_id=v_agent
  ...
  if v_task.lease_agent_id is not null and (lower(v_task.lease_agent_id)<>v_agent or v_task.lease_tab_id<>p_tab
     or lower(v_task.lease_target_id)<>v_target or v_task.lease_agent_generation_epoch<>p_epoch) then
  ...
  if v_class='PRE_EFFECT_ABORTED' then
    ...
    update destruktion_meta.devos_fleet_task_h205f22
       set state='READY',lease_agent_id=null,lease_tab_id=null,lease_target_id=null,lease_agent_generation_epoch=null,
           lease_expires_at=null,result_checkpoint_id=null,result_summary=null,result_summary_sha256=null,error_code=null,updated_at=v_now
  ...
  if v_proven_at>v_last_seen+interval '5 seconds' then raise exception 'devos_ambiguity_transport_time_fenced'; end if;

  if v_task.state='RUNNING' and lower(coalesce(v_task.lease_agent_id,''))=v_agent and v_task.lease_tab_id=p_tab
     and lower(coalesce(v_task.lease_target_id,''))=v_target and v_task.lease_agent_generation_epoch=p_epoch
  ...
  if exists(select 1 from destruktion_meta.devos_fleet_claim_h205f22 c where c.workspace_id=p_workspace and c.state='ACTIVE' and c.expires_at>v_now
      and (c.task_id=p_task or c.agent_id=v_agent) and not(c.task_id=p_task and c.agent_id=v_agent and c.lease_generation=p_generation
        and c.tab_id=p_tab and lower(c.target_id)=v_target and c.agent_generation_epoch=p_epoch)) then
    raise exception 'devos_ambiguity_active_claim_conflict_fenced';
  end if;
  update destruktion_meta.devos_fleet_claim_h205f22 set state='FENCED',updated_at=v_now
   where workspace_id=p_workspace and task_id=p_task and lease_generation=p_generation and state='ACTIVE';
  update destruktion_meta.devos_fleet_task_h205f22
     set state='RUNNING',lease_agent_id=v_agent,lease_tab_id=p_tab,lease_target_id=v_target,lease_agent_generation_epoch=p_epoch,
         lease_expires_at=v_claim_expires,result_checkpoint_id=null,result_summary=null,result_summary_sha256=null,error_code=null,updated_at=v_now
   where task_id=p_task and workspace_id=p_workspace and state='AMBIGUOUS' and lease_generation=p_generation;
  if not found then raise exception 'devos_ambiguity_effect_proven_transition_fenced'; end if;
  insert into destruktion_meta.devos_fleet_claim_h205f22(claim_id,task_id,workspace_id,point_id,role,agent_id,claim_class,lease_generation,base_sha,tab_id,target_id,agent_generation_epoch,state,expires_at,created_at,updated_at,authority_effect)
  values(gen_random_uuid(),p_task,p_workspace,v_task.point_id,v_task.role,v_agent,v_claim.claim_class,p_generation,v_task.base_sha,p_tab,v_target,p_epoch,'ACTIVE',v_claim_expires,v_now,v_now,false);
...
-- v1 remains in migration history but is no longer callable through the privileged service role.
revoke execute on function public.devos_fleet_reconcile_ambiguous_v1(uuid,uuid,text,bigint,text,text,bigint,jsonb) from service_role;
