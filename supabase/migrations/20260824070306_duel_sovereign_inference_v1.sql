-- METAENGINE H205F22 SOVEREIGN INFERENCE V1
-- Makes the duel plane runnable without managed inference billing gates.
-- SOVEREIGN_ONLY sessions are woken through PostgreSQL LISTEN/NOTIFY and are
-- fenced away from Cloudflare-hosted workers. Hosted wake remains for ANY/HOSTED_ONLY.

create or replace function public.h205f22_duel_create_sovereign_v1(
  p_duel_key text,
  ...
  p_max_ticks integer default 64
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  s := coalesce(p_subject,'{}'::jsonb) || jsonb_build_object(
    ...
    'inference_class','OPEN_WEIGHT_SELF_HOSTED',
    'tariff_dependency',false,
    'gpt_model_identity',p_gpt_model,
    'glm_model_identity',p_glm_model
  );
  r := public.h205f22_duel_create_lockstep_v2(
    p_duel_key,p_milestone_key,p_base_github_sha,s,p_gpt_model,p_glm_model,p_max_ticks
  );
...
revoke all on function public.h205f22_duel_create_sovereign_v1(text,text,text,jsonb,text,text,integer) from public,anon,authenticated;
grant execute on function public.h205f22_duel_create_sovereign_v1(text,text,text,jsonb,text,text,integer) to service_role;
...
begin
  ...
  if d.status='READY' then
    ...
    return jsonb_build_object('schema','metaengine.compute.duel-target-lease-read.h205f22.v3','leased',false,'reason','BUSY','duel_id',d.duel_id,'status',d.status,'execution_policy',v_policy,'canonical',false,'authority_effect',false);
  else
    return jsonb_build_object('schema','metaengine.compute.duel-target-lease-read.h205f22.v3','leased',false,'reason','TERMINAL','duel_id',d.duel_id,'status',d.status,'execution_policy',v_policy,'canonical',false,'authority_effect',false);
  end if;

  v_readback := public.h205f22_duel_read_lockstep_v2(p_duel_id,p_after_tick);

  return jsonb_build_object(
    'schema','metaengine.compute.duel-target-lease-read.h205f22.v3',
    'leased',true,
    ...
    'readback',v_readback,'execution_policy',v_policy,
...
begin
  ...
  if not found then
    return jsonb_build_object('schema','metaengine.compute.duel-lockstep-lease.h205f22.v2','leased',false);
  end if;

  update destruktion_meta.compute_fabric_duel_session_h205f22
  set status='RUNNING',lease_owner=p_worker,lease_generation=lease_generation+1,
      lease_expires_at=clock_timestamp()+make_interval(secs=>p_lease_seconds),updated_at=clock_timestamp()
  where duel_id=d.duel_id
  returning * into d;

  return jsonb_build_object(
    'schema','metaengine.compute.duel-lockstep-lease.h205f22.v2','leased',true,
    'duel_id',d.duel_id,'duel_key',d.duel_key,'milestone_key',d.milestone_key,
    'checkpoint_id',d.semantic_checkpoint_id,'payload_root_sha256',d.semantic_payload_root_sha256,
    'base_github_sha',d.base_github_sha,'subject',d.subject,'gpt_model',d.gpt_model,'glm_model',d.glm_model,
    'protocol_version',d.protocol_version,'current_tick',d.current_tick,
    'current_checkpoint_sha256',d.current_checkpoint_sha256,'max_ticks',d.max_ticks,
    'lease_generation',d.lease_generation,'lease_expires_at',d.lease_expires_at,
...
begin
  ...
  perform pg_notify(
    'h205f22_duel_ready_v1',
    jsonb_build_object(
      'duel_id',d.duel_id,
      'duel_key',d.duel_key,
      'checkpoint_sha256',d.current_checkpoint_sha256,
      'execution_policy',v_policy
    )::text
  );

  notify_evt := jsonb_build_object(
    'wake_id',wake_id,
    'channel','h205f22_duel_ready_v1',
    'execution_policy',v_policy,
    'canonical',false,
    'authority_effect',false
  );
  insert into destruktion_meta.compute_fabric_duel_event_h205f22(duel_id,phase,actor,payload,payload_sha256)
  values(d.duel_id,'SOVEREIGN_WAKE_NOTIFIED','SYSTEM',notify_evt,
    encode(extensions.digest(convert_to(notify_evt::text,'utf8'),'sha256'),'hex'));

  if v_policy='SOVEREIGN_ONLY' then
    return null;
  end if;

  select runtime_url into u
  from destruktion_meta.compute_fabric_aop_runtime_endpoint_h205f22
  where endpoint_key='PRIMARY';
  if u is null then return null; end if;

  select decrypted_secret into k
  from vault.decrypted_secrets
  where name='aop1_wake_secret'
  order by created_at desc limit 1;
  if k is null or length(k)<16 then raise exception 'duel_wake_secret_unavailable'; end if;

  body:=jsonb_build_object(
    'id',wake_id,'reason','DUEL_DB_INSERT','source','supabase-pg-net',
    'payload',jsonb_build_object('duel_id',d.duel_id,'duel_key',d.duel_key,'checkpoint_sha256',d.current_checkpoint_sha256)
  );
  ...
  select net.http_post(
    url:=u||'/duel/db-wake',
    body:=body,
    headers:=jsonb_build_object('Content-Type','application/json','X-Metaengine-Duel-Signature-256','sha256='||sig),
    timeout_milliseconds:=5000
  ) into req;
  evt:=jsonb_build_object(
    'request_id',req,
    'runtime_url_sha256',encode(extensions.digest(convert_to(u,'utf8'),'sha256'),'hex'),
    'wake_id',wake_id,
    'message_sha256',encode(extensions.digest(convert_to(msg,'utf8'),'sha256'),'hex'),
    'canonical',false,'authority_effect',false
  );
  insert into destruktion_meta.compute_fabric_duel_event_h205f22(duel_id,phase,actor,payload,payload_sha256)
  values(d.duel_id,'WAKE_ENQUEUED','SYSTEM',evt,
    encode(extensions.digest(convert_to(evt::text,'utf8'),'sha256'),'hex'));
  return req;
end
$function$;
