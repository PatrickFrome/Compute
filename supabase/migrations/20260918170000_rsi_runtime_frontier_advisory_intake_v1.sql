...
begin
  ...
  if jsonb_typeof(v_frontier) <> 'array'
     or jsonb_array_length(v_frontier) > 4
     or coalesce(v_rsi ->> 'frontier_count','') !~ '^[0-4]  loop
    v_seen := v_seen + 1;
    ...
    if jsonb_typeof(v_hypothesis) <> 'object'
       ...
       or coalesce((v_hypothesis #>> '{acceptance_contract,paired_parent_candidate_required}')::boolean,false) is not true
       or coalesce((v_hypothesis #>> '{acceptance_contract,holdout_required}')::boolean,false) is not true
       or coalesce((v_hypothesis #>> '{acceptance_contract,no_optional_stopping}')::boolean,false) is not true
       or coalesce((v_hypothesis #>> '{acceptance_contract,scalar_reward_authoritative}')::boolean,true) is not false
       or coalesce((v_hypothesis #>> '{acceptance_contract,candidate_authored_receipts_allowed}')::boolean,true) is not false
    ...
    if coalesce(v_hypothesis ->> 'opportunity_id','') <> v_opportunity_id
       or coalesce(v_plan ->> 'hypothesis_id','') <> coalesce(v_hypothesis ->> 'hypothesis_id','')
       ...
       or lower(coalesce(v_plan #>> '{task_spec,rsi,source_sha}','')) <> v_source_sha
       or coalesce(v_entry ->> 'observation_digest','') <> coalesce(v_hypothesis ->> 'observation_digest','')
       or coalesce(v_plan #>> '{task_spec,rsi,observation_digest}','') <> coalesce(v_hypothesis ->> 'observation_digest','')
       or upper(coalesce(v_entry ->> 'signal','')) <> upper(coalesce(v_hypothesis ->> 'signal',''))
       or upper(coalesce(v_plan #>> '{task_spec,rsi,signal}','')) <> upper(coalesce(v_hypothesis ->> 'signal',''))
       or upper(coalesce(v_entry ->> 'mutation_surface','')) <> upper(coalesce(v_hypothesis ->> 'mutation_surface',''))
       or upper(coalesce(v_plan #>> '{task_spec,rsi,mutation_surface}','')) <> upper(coalesce(v_hypothesis ->> 'mutation_surface','')) then
...
grant execute on function public.h205f22_rsi_runtime_frontier_intake_v1(uuid,text,jsonb) to service_role;
     ...
     or (v_rsi ->> 'frontier_count')::integer <> jsonb_array_length(v_frontier) then
    raise exception 'rsi_frontier_intake_capacity_invalid';
  end if;

  select count(*) into v_active_reviews
    from destruktion_meta.devos_fleet_task_h205f22 t
   where t.workspace_id = p_workspace
     and t.task_spec ->> 'source' = 'NATIVE_BROWSER_RSI_FRONTIER'
     and t.task_spec ->> 'source_client_id' = lower(p_client)
     and t.state in ('READY','LEASED','RUNNING','RESULT_READY','BLOCKED','AMBIGUOUS');

  if v_active_reviews >= 16 then
    return jsonb_build_object(
      'accepted',true,
      'state','ADVISORY_CAPACITY_FENCED',
      'source_sha',v_source_sha,
      'active_reviews',v_active_reviews,
      'seen',0,
      'enqueued',0,
      'duplicates',0,
      'scheduler','DEVOS_EXISTING_ONLY',
