...
alter table public.compute_fabric_a2_browser_supervisor_command_h205f22
  add constraint a2_browser_supervisor_command_action_ck check (action = any (array[
    ...
    'SELF_UPDATE_STATUS','SELF_UPDATE_CHECK','SELF_UPDATE_DOWNLOAD','SELF_UPDATE_INSTALL','SELF_UPDATE_RETRY','SELF_UPDATE_SET_AUTOMATIC'
  ]::text[]));
...
begin
  ...
  if p_payload is null or jsonb_typeof(v_payload)<>'object' or octet_length(v_payload::text)>131072 then raise exception 'native_supervisor_payload_invalid'; end if;

  if v_action like 'SELF_UPDATE_%' then
    if v_platform is not null then raise exception 'native_supervisor_self_update_platform_must_be_null'; end if;
    if v_action='SELF_UPDATE_SET_AUTOMATIC' then
      select array_agg(k order by k) into v_payload_keys from jsonb_object_keys(v_payload) as x(k);
      if v_payload_keys is distinct from array['enabled']::text[]
         or jsonb_typeof(v_payload->'enabled') is distinct from 'boolean' then
        raise exception 'native_supervisor_self_update_automatic_payload_invalid';
      end if;
    elsif v_payload <> '{}'::jsonb then
      raise exception 'native_supervisor_self_update_payload_must_be_empty';
    end if;
  end if;
  ...
  insert into public.compute_fabric_a2_browser_supervisor_command_h205f22(
    ...
    v_command_id,'2de9f84b-7c0a-4091-911c-894ff1d6eaf4'::uuid,v_client,v_issued_by,v_action,v_platform,v_payload,'PENDING',
...
begin
  ...
  select coalesce(sum(case
      when action in ('POLL','CAPTURE','SELF_UPDATE_STATUS','DISARM') then 0
      when action='SET_SUPERVISOR_MODE' and upper(coalesce(payload->>'mode',''))='OFF' then 0
      when action in ('SCROLL','SEMANTIC_FOCUS','SELF_UPDATE_CHECK') then 1
      when action in ('ARM','SET_SUPERVISOR_MODE','SET_MODE','STOP_GENERATION','SELF_UPDATE_RETRY','SELF_UPDATE_SET_AUTOMATIC') then 2
      when action in ('SEMANTIC_TYPE','SELF_UPDATE_DOWNLOAD') then 3
      when action in ('RESOLVE_PROMPT','TYPED_CLICK','SELF_UPDATE_INSTALL') then 4
  ...
  where workspace_id=p_workspace_id and status='PENDING' and expires_at>v_now
    ...
    and (v_mode='CONTROL' or action in ('SET_SUPERVISOR_MODE','ARM','DISARM','SELF_UPDATE_STATUS'))
  ...
  v_requested_cost := case
    when v_row.action in ('POLL','CAPTURE','SELF_UPDATE_STATUS','DISARM') then 0
    when v_row.action='SET_SUPERVISOR_MODE' and upper(coalesce(v_row.payload->>'mode',''))='OFF' then 0
    when v_row.action in ('SCROLL','SEMANTIC_FOCUS','SELF_UPDATE_CHECK') then 1
    when v_row.action in ('ARM','SET_SUPERVISOR_MODE','SET_MODE','STOP_GENERATION','SELF_UPDATE_RETRY','SELF_UPDATE_SET_AUTOMATIC') then 2
    when v_row.action in ('SEMANTIC_TYPE','SELF_UPDATE_DOWNLOAD') then 3
    when v_row.action in ('RESOLVE_PROMPT','TYPED_CLICK','SELF_UPDATE_INSTALL') then 4
    else 4 end;
...
begin
  ...
  v_effect := coalesce(p_ok,false) and v_row.action in (
    ...
    'NEW_TAB','SELECT_TAB','CLOSE_TAB','NAVIGATE','BACK','FORWARD','RELOAD',
    'FLEET_RECONCILE','FLEET_SET_PROFILE',
    'SELF_UPDATE_CHECK','SELF_UPDATE_DOWNLOAD','SELF_UPDATE_INSTALL','SELF_UPDATE_RETRY','SELF_UPDATE_SET_AUTOMATIC'
  );
