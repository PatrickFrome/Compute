...
async function latestActionRows(){const r=await pool.query<Row>(`select distinct on(semantic_point,agent) event_hash,agent,event_type,payload,commit_seq,semantic_point from destruktion_meta.compute_fabric_a2_agent_event_h205f22 where workspace_id=$1 and event_type in ('ACTION_PROPOSAL','CLAIM','COUNTERCLAIM') and nullif(btrim(coalesce(payload->'proposed_action'->>'kind','')),'') is not null order by semantic_point,agent,commit_seq desc`,[WORKSPACE_ID]);return r.rows;}
