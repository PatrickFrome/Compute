...
export async function runMicrostepDuel(env:Env,step:WorkflowStep,workerId:string):Promise<unknown>{
  ...
  try{
    for(let tick=lastTick+1;tick<=max;tick++){
      const rt=await step.do(`microstep-read-${tick}`,async()=>JSON.stringify(await rpc<Readback>(env,"h205f22_duel_read_lockstep_v2",{p_duel_id:lease.duel_id,p_after_tick:Math.max(0,tick-9)})));
      ...
      const pt=await step.do(`microstep-persist-${tick}`,{retries:{limit:3,delay:"1 second",backoff:"exponential"}},async()=>JSON.stringify(await rpc<JsonObject>(env,"h205f22_duel_submit_pair_v2",{p_duel_id:lease.duel_id,p_worker:workerId,p_lease_generation:lease.lease_generation,p_tick_no:tick,p_seen_checkpoint_sha256:checkpoint,p_gpt_step_type:st(pair.g),p_gpt_payload:pair.g,p_glm_step_type:st(pair.l),p_glm_payload:pair.l})));
