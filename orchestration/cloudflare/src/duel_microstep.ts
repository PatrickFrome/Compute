...
async function withDeadline<T>(ms:number,fn:(signal:AbortSignal)=>Promise<T>):Promise<T>{const c=new AbortController();const id=setTimeout(()=>c.abort("duel_model_timeout"),ms);try{return await fn(c.signal);}finally{clearTimeout(id);}}
...
async function actor(env:Env,a:Actor,l:Lease,p:string):Promise<JsonObject>{const ms=timeoutMs(env);return withDeadline(ms,async(signal)=>{if(env.VERCEL_AI_GATEWAY_API_KEY){return vercel(env,a==="GPT"?(l.gpt_model||GPT):(l.glm_model||GLM_VERCEL),p,signal);}return a==="GPT"?gptCloudflare(env,l.gpt_model||GPT,p,signal):glmCloudflare(env,l.glm_model||GLM_CF,p,signal);});}
function visibleExecutorError(a:Actor,error:unknown,peerHash:string|null,backend:string):JsonObject{return{step_type:"EXECUTOR_ERROR",summary:`${a} execution slot did not return a model microstep`,evidence_used:[],peer_event_hash_addressed:peerHash,action:{kind:"BLOCKED_EXECUTOR",backend},falsifier:"A later exact-model invocation succeeds under the same immutable subject",risk_delta:"No model reasoning was fabricated; this is a SYSTEM-observed executor failure.",ready_to_resolve:false,terminal_vote:null,need_canary:false,resolution:null,synthetic:true,model_response:false,error_class:String(error).includes("AbortError")||String(error).includes("duel_model_timeout")?"TIMEOUT":"PROVIDER_ERROR",error:String(error).slice(0,1200),canonical:false,authority_effect:false};}
async function actorVisible(env:Env,a:Actor,l:Lease,p:string,peerHash:string|null):Promise<ActorResult>{try{return{payload:await actor(env,a,l,p),executorError:false};}catch(error){return{payload:visibleExecutorError(a,error,peerHash,env.VERCEL_AI_GATEWAY_API_KEY?"VERCEL_AI_GATEWAY":"CLOUDFLARE_AI"),executorError:true};}}
...
export async function runMicrostepDuel(env:Env,step:WorkflowStep,workerId:string):Promise<unknown>{
  ...
  try{
    for(let tick=lastTick+1;tick<=max;tick++){
      ...
      if(ready&&gv&&lv&&gv===lv){const result:JsonObject={schema:"metaengine.compute.duel-microstep-result.h205f22.v2",outcome:"RESOLVED",winner:gv,inference_backend:env.VERCEL_AI_GATEWAY_API_KEY?"VERCEL_AI_GATEWAY":"CLOUDFLARE_AI",final_tick:tick,final_checkpoint_sha256:checkpoint,gpt_resolution:pair.g.payload.resolution??null,glm_resolution:pair.l.payload.resolution??null,canonical:false,authority_effect:false};return await step.do(`microstep-complete-${tick}`,async()=>JSON.stringify(await rpc<JsonObject>(env,"h205f22_duel_complete_lockstep_v2",{p_duel_id:duelId,p_worker:workerId,p_lease_generation:leaseGeneration,p_status:"RESOLVED",p_result:result})));}
      if(pair.g.payload.need_canary===true&&pair.l.payload.need_canary===true){const result:JsonObject={schema:"metaengine.compute.duel-microstep-result.h205f22.v2",outcome:"CANARY_REQUIRED",final_tick:tick,final_checkpoint_sha256:checkpoint,gpt:pair.g.payload,glm:pair.l.payload,canonical:false,authority_effect:false};return await step.do(`microstep-canary-${tick}`,async()=>JSON.stringify(await rpc<JsonObject>(env,"h205f22_duel_complete_lockstep_v2",{p_duel_id:duelId,p_worker:workerId,p_lease_generation:leaseGeneration,p_status:"CANARY_REQUIRED",p_result:result})));}
    }
