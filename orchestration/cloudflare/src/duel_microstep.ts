...
type Readback = JsonObject & { status?:string; current_tick?:number; current_checkpoint_sha256?:string; events?:JsonObject[]; ticks?:JsonObject[] };
type PairReceipt = JsonObject & { persisted_readback?:boolean; output_checkpoint_sha256?:string; gpt_event?:JsonObject; glm_event?:JsonObject; tick?:JsonObject };
...
function appendPersisted(read:Readback,receipt:PairReceipt,tick:number,checkpoint:string):Readback{
  if(receipt.persisted_readback!==true||!receipt.gpt_event||!receipt.glm_event||!receipt.tick)throw new Error(`duel_pair_persisted_readback_required:${tick}`);
  const events=[...(Array.isArray(read.events)?read.events:[]),asObj(receipt.gpt_event),asObj(receipt.glm_event)].slice(-18);
  const ticks=[...(Array.isArray(read.ticks)?read.ticks:[]),asObj(receipt.tick)].slice(-9);
...
export async function runMicrostepDuel(env:Env,step:WorkflowStep,workerId:string):Promise<unknown>{
  ...
  const initialText=await step.do("microstep-read-initial",async()=>JSON.stringify(await rpc<Readback>(env,"h205f22_duel_read_lockstep_v2",{p_duel_id:duelId,p_after_tick:Math.max(0,lastTick-8)})));
