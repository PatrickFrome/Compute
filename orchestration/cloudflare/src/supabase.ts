...
export async function completeRun(env: Env, lease: AopLease, workerId: string, resultCode: string, output: Record<string, unknown>, githubSha?: string | null, wakeCondition?: string | null): Promise<Record<string, unknown>> {
...
export async function deferRun(env: Env, lease: AopLease, workerId: string, condition: string, reason: Record<string, unknown>): Promise<Record<string, unknown>> {
  if (!lease.run_id || lease.lease_generation == null) throw new Error("invalid_lease");
...
export async function supervisorAdoptClaim(env: Env, lease: AopLease, workerId: string): Promise<Record<string, unknown>> {
...
export async function supervisorReturnAuthority(env: Env, lease: AopLease, workerId: string, instructions: Record<string, unknown>): Promise<Record<string, unknown>> {
  if (!env.AOP_SUPERVISOR_TOKEN) throw new Error("supervisor_token_unavailable");
  if (!lease.run_id || lease.lease_generation == null) throw new Error("invalid_lease");
