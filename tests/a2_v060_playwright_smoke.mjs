...
const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'a2-v060-'));
...
try {
  ...
  if (facts.version !== '0.6.0') throw new Error(`manifest version ${facts.version}`);
  if (facts.name !== 'METAENGINE A2 Browser Operator') throw new Error(`manifest name ${facts.name}`);
  if (facts.runtime !== '0.6.0-dev.1') throw new Error(`operator runtime ${facts.runtime}`);
  for (const key of ['glm', 'gpt', 'request', 'reconcile', 'vault']) {
  ...
  console.log('A2 Browser Operator v0.6 MV3 runtime: PASS', JSON.stringify(facts));
} finally {
  await context.close();
