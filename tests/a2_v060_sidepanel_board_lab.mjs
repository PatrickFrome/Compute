...
const trustedSupervisorImport='importScripts("./trusted-supervisor-chat-v063.js");';
...
const trustedSupervisorIndex=entry.indexOf(trustedSupervisorImport);
...
if(sessionIndex>=0){
  ...
  if(trustedSupervisorIndex>=0) assert.ok(trustedSupervisorIndex>sessionIndex,'trusted supervisor chat must load after its session manager');
}
...
if(activeClientName==='supervisor-client-v063-authority.js'){
  ...
  assert.ok(sessionIndex>=0,'v0.6.3 authority runtime requires the self-healing supervisor session manager');
}

console.log('a2_v060_sidepanel_board_lab: PASS',{requiredIds:required.size,activeClientName,sessionManager:sessionIndex>=0,trustedSupervisorChat:trustedSupervisorIndex>=0});