...
const version = manifest.version.split('.').map((part) => Number.parseInt(part, 10));
const versionAtLeast062 = version.length === 3 && version.every(Number.isInteger)
  && (version[0] > 0 || version[1] > 6 || (version[1] === 6 && version[2] >= 2));
check('manifest preserves Browser Operator v0.6.2+ contract', versionAtLeast062 && Number(manifest.minimum_chrome_version) >= 125);
...
console.log(`\n${passed}/${checks.length} Browser Operator adversarial DOM/operator checks passed`);
