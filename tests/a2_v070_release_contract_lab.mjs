...
const observability = read('target-observability.js');
...
assert.equal(pkg.operator_runtime, '0.7.0-dev.2');
assert.equal(extensionIdFromKey(manifest.key), 'bgklkgkbpkgnjnjjkmemdgmekhhejglo');
assert.equal(manifest.incognito, 'not_allowed');

assert.match(entry, /importScripts\("\.\/runtime-marker\.js"\);\s*importScripts\("\.\/target-registry\.js"\);\s*importScripts\("\.\/target-observability\.js"\);\s*importScripts\("\.\/bridge-runtime\.js"\);/s);
assert.doesNotMatch(entry, /-v\d{3}\.js/);
assert.ok(pkg.files.includes('target-registry.js'));
assert.ok(pkg.files.includes('target-observability.js'));
...
assert.match(runtimeMarker, /const RUNTIME = "0\.7\.0-dev\.2"/);
assert.match(runtimeMarker, /milestone: "R3_TARGET_REGISTRY_V1"/);
const runtimeOwners = pkg.files.filter((name) => fs.readFileSync(path.join(root, name), 'utf8').includes('0.7.0-dev.2'));
...
for (const marker of [
  ...
  'a2TargetRegistryV1',
  'a2TargetBindingsV1',
  'a2TargetRegistryLastError',
  'browser_session_nonce',
  'gpt_primary',
  'glm_primary',
  'conversation_epoch',
  'target_registry_duplicate_active_url',
  'target_registry_legacy_url_conflict',
  'LEGACY_URL_CONFLICT',
...
for (const marker of [
  'metaengine.a2-browser-operator.target-observability.v1',
  'a2TargetObservabilityV1',
  'health_fresh_for_epoch',
  'tainted_page_data: true',
  'authority_effect: false',
  'Passive observer only',
  'A2_TARGET_OBSERVABILITY_LIST'
]) assert.ok(observability.includes(marker), `target observability invariant missing: ${marker}`);
assert.doesNotMatch(observability, /messages\s*:/, 'observability must not persist messages');
assert.doesNotMatch(observability, /composer_text\s*:/, 'observability must not persist composer text');
assert.doesNotMatch(observability, /document\.|querySelector|innerText|textContent/, 'observability must not inspect page DOM');
assert.doesNotMatch(observability, /eval\s*\(|new Function|executeScript/, 'observability must not execute arbitrary code');
