...
CF_INDEX = (ROOT / "orchestration/cloudflare/src/index.ts").read_text(encoding="utf-8")
CF_SUPABASE = (ROOT / "orchestration/cloudflare/src/supabase.ts").read_text(encoding="utf-8")
...
# Role proxies pin the configured exact model instead of allowing arbitrary client model selection.
...
assert "AsyncIterable<Uint8Array>" in CONTROL  # streamed proxy; no full-response buffering

# Local control creates/reads/signals V4 using the existing fenced DB protocol.
...
# npm start supervises both the coordinator and the endpoint gateway.
...
# Cloudflare is optional remote control only: create/read are allowlisted, execution/finalize are not.
assert '"h205f22_duel_create_same_point_v4"' in CF_SUPABASE
assert '"h205f22_duel_read_same_point_v4"' in CF_SUPABASE
assert '"h205f22_duel_submit_rebut_finalize_v4"' not in CF_SUPABASE
assert '"h205f22_duel_finalize_same_point_v4"' not in CF_SUPABASE
assert 'url.pathname === "/v4/health"' in CF_INDEX
assert 'url.pathname === "/v4/duels"' in CF_INDEX
assert 'mode: "CONTROL_ONLY"' in CF_INDEX
assert 'executor: "FENCED"' in CF_INDEX
assert "same_point_v4_executor: false" in CF_INDEX
