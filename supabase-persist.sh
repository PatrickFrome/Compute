...
# Креды из /tmp/my-project/.a2-backup/me2.env.20260922 (PolarFS, пережил reset).
# v1.1: контроль HTTP-кодов (200 обязательен), POST-листинг для верификации.
...
DEDUP="/tmp/.sbx-dedup"
LOG="$VAULT/journal/supabase-persist.log"
...
upload() { # $1=local file $2=object name
  ...
  if [ "$code" = "200" ]; then
    ...
    echo "[$(date +%Y%m%d-%H%M%S)] UP $obj ($(stat -c%s "$f")B)" >> "$LOG"
  else
    echo "[$(date +%Y%m%d-%H%M%S)] UP-FAIL $obj HTTP=$code" >> "$LOG"
  fi
}
...
N="$(curl -s -X POST -H "Authorization: Bearer $SJ" -H "apikey: $SJ" \
  ...
  "$SU/storage/v1/object/list/me2-evidence" 2>/dev/null | grep -o '"name"' | wc -l | tr -d ' ')"
echo "supabase-persist ok: $N objects verified in context-vault/"
