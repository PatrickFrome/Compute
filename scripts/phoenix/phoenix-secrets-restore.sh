#!/usr/bin/env bash
# phoenix-secrets-restore.sh v1.0 — post-reset secrets self-restore ORCHESTRATOR (repo-safe: NO secret literals).
#
...
# SEALED bootstrap on surviving channels (PolarFS /tmp + mirrors). After an env-reset:
...
#   3) it validates the PAT and prints next steps (push-pending / guard / union-merge)
set -u
...
main() {
  if have_env; then
    ...
    if [ "$C" = "200" ]; then echo "SECRETS: OK — nothing to restore"; exit 0; fi
